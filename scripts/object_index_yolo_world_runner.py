#!/usr/bin/env python3
"""YOLO-World adapter for object-index stage."""
...
def _run_yolo_world(payload: Mapping[str, Any]) -> Dict[str, Any]:
...
def main(argv: List[str] | None = None) -> int:
    ...
    if len(args) != 2:
        print("usage: object_index_yolo_world_runner.py <input_json> <output_json>", file=sys.stderr)
        return 2
    input_path = Path(args[0])
    output_path = Path(args[1])
    payload = _read_payload(input_path)
    result = _run_yolo_world(payload)
