...
export COSMOS_MODEL_REVISION="${COSMOS_MODEL_REVISION:-0d37c7498f54cee3c599d438d895a0a4a8608064}"
...
export BLUEPRINT_NATIVE_RUNTIME_BACKEND="${BLUEPRINT_NATIVE_RUNTIME_BACKEND:-cosmos_wam}"
...
if [ -z "${BLUEPRINT_RUNTIME_AUTH_TOKEN:-${SITE_WORLD_RUNTIME_SERVICE_API_KEY:-}}" ] && \
   [ -z "${BLUEPRINT_RUNTIME_AUTH_TOKENS_JSON:-}" ]; then
  echo "[start-native-runtime-vast] ERROR: configure a nonempty runtime bearer token" >&2
  echo "[start-native-runtime-vast] Set SITE_WORLD_RUNTIME_SERVICE_API_KEY, BLUEPRINT_RUNTIME_AUTH_TOKEN, or BLUEPRINT_RUNTIME_AUTH_TOKENS_JSON." >&2
  exit 1
fi
