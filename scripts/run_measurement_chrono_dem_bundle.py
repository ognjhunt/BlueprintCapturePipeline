#!/usr/bin/env python3
"""Run a digest-bound Chrono::DEM CUDA development bundle."""
...
from blueprint_pipeline.measurement_chrono_dem_cuda_adapter import EXPECTED_SOURCE_COMMIT
from blueprint_pipeline.measurement_chrono_dem_runtime_release import RUNTIME_IMAGE
...
def _read(path: Path) -> dict:
    ...
    if not isinstance(value, dict):
        raise ValueError("measurement_chrono_dem_bundle_object_required")
    return value
...
def _environment(name: str) -> str:
    ...
    if not value:
        raise ValueError(f"measurement_chrono_dem_environment_missing:{name}")
    return value
...
def run_bundle(bundle_root: Path) -> dict:
    ...
    blockers: list[str] = []
    if manifest.get("schema_version") != "measurement_chrono_dem_cuda_input_bundle.v1":
        blockers.append("measurement_chrono_dem_bundle_manifest_schema_invalid")
    ...
    if manifest_digest != canonical_digest(manifest, digest_field="bundle_manifest_digest"):
        blockers.append("measurement_chrono_dem_bundle_manifest_digest_mismatch")
    bindings = {
        "source_commit_sha": _environment("BLUEPRINT_MEASUREMENT_CHRONO_DEM_SOURCE_COMMIT"),
        "runtime_image_digest": _environment("BLUEPRINT_MEASUREMENT_CHRONO_DEM_RUNTIME_IMAGE"),
        "runtime_release_digest": _environment(
            "BLUEPRINT_MEASUREMENT_CHRONO_DEM_RUNTIME_RELEASE_DIGEST"
        ),
        "input_bundle_digest": _environment("BLUEPRINT_MEASUREMENT_CHRONO_DEM_INPUT_BUNDLE_DIGEST"),
    }
    for key in ("source_commit_sha", "runtime_image_digest", "runtime_release_digest"):
        if manifest.get(key) != bindings[key]:
            blockers.append(f"measurement_chrono_dem_bundle_{key}_mismatch")
    if manifest.get("runtime_image_digest") != RUNTIME_IMAGE:
        blockers.append("measurement_chrono_dem_bundle_image_mismatch")
    if (
        manifest.get("chrono_source_commit") != EXPECTED_SOURCE_COMMIT
        or manifest.get("required_backend") != "cuda"
        or manifest.get("replay_count") != 2
    ):
        blockers.append("measurement_chrono_dem_bundle_runtime_contract_invalid")
    for record in manifest.get("source_files") or []:
        if not isinstance(record, dict):
            blockers.append("measurement_chrono_dem_bundle_source_record_invalid")
            continue
        path = bundle_root / str(record.get("path") or "")
        if not path.is_file() or path.is_symlink() or _sha256(path) != record.get("digest"):
            blockers.append(
                f"measurement_chrono_dem_bundle_source_digest_mismatch:{record.get('path')}"
            )
    requests: list[dict] = []
    request_files = manifest.get("request_files")
    if not isinstance(request_files, list) or len(request_files) != 2:
        blockers.append("measurement_chrono_dem_bundle_request_files_invalid")
    else:
        for record in request_files:
            try:
                ...
                blockers.append(
                    f"measurement_chrono_dem_bundle_request_invalid:{type(exc).__name__}"
                )
                continue
            if request["execution_request_digest"] != record.get("execution_request_digest"):
                blockers.append("measurement_chrono_dem_bundle_request_digest_mismatch")
            ...
            bundles.append(
                run_measurement_adapter_execution(
                    ...
                    command_argv=[
                        sys.executable,
                        "-m",
                        "blueprint_pipeline.measurement_chrono_dem_cuda_adapter",
                    ],
                    execute=True,
                )
            )
    ...
    if completed and any(
        ...
        or row.get("chrono_dem_module_used") is not True
        or row.get("cuda_available") is not True
        or row.get("cuda_device_count") != 1
        or row.get("cpu_fallback_used") is not False
        or row.get("deterministic_replay_match") is not True
        for row in observations
    ):
        blockers.append("measurement_chrono_dem_runtime_observation_invalid")
    metrics = [
    ...
    if completed and len(metrics) == 2:
        spreads = [float(row["state_trajectory"]) for row in metrics]
        forces = [float(row["force"]) for row in metrics]
        aggregate = {
            "case_count": 2,
            "minimum_spread_ratio": min(spreads),
            "maximum_spread_ratio": max(spreads),
            "mean_ground_reaction_force_n": fmean(forces),
            "within_envelope_case_count": sum(
                row.get("task_outcome") == "within_chrono_dem_cuda_synthetic_envelope"
                for row in metrics
            ),
        }
...
def main() -> int:
    ...
    try:
        ...
        result = {
            "schema_version": RUNTIME_RESULT_SCHEMA_VERSION,
            "status": "failed",
            "blockers": [f"measurement_chrono_dem_bundle_controller_failure:{type(exc).__name__}"],
            "raw_secret_values_recorded": False,
            "proof_effect": "none",
