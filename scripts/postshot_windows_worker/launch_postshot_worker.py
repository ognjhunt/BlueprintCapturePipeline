...
def launch(arguments: argparse.Namespace) -> None:
    proxy_root = Path(arguments.proxy_root).resolve()
    state_dir = _state_dir(proxy_root, arguments.run_id)
    admission = _read_json(state_dir / "admission.json")
    if admission.get("launch_allowed") is not True:
        raise SystemExit("paid_launch_blocked_by_admission")
    staging = _read_json(state_dir / "staging.json")
    urls = json.loads((state_dir / "presigned_urls.json").read_text(encoding="utf-8"))
    ttl_deadline = utc_now_iso(time.time() + ATTEMPT_5_TTL_MINUTES * 60)
    prelaunch_inventory = _inventory_exact(run_id=arguments.run_id, launch={})
    prelaunch_proof = build_provider_zero_proof(run_id=arguments.run_id, region=REGION, instances=prelaunch_inventory.get("instances", []), volumes=prelaunch_inventory.get("volumes", []), snapshots=prelaunch_inventory.get("snapshots", []), images=prelaunch_inventory.get("images", []), elastic_ips=prelaunch_inventory.get("elastic_ips", []), security_groups=prelaunch_inventory.get("security_groups", []), checked_at_utc=utc_now_iso())
    _write_artifact(state_dir / "prelaunch_provider_zero.json", prelaunch_proof)
    if prelaunch_proof["provider_zero"] is not True:
        raise SystemExit("paid_launch_blocked_provider_zero_not_proven")
    watchdog = _spawn_external_watchdog(state_dir=state_dir, run_id=arguments.run_id, ttl_deadline_utc=ttl_deadline)
    session = _aws_session()
    ec2 = session.client("ec2")
    sts = session.client("sts")
    account_id = sts.get_caller_identity().get("Account")
    groups = ec2.describe_security_groups(Filters=[{"Name": "group-name", "Values": [TAG]}]).get("SecurityGroups", [])
    if groups:
        group_id = groups[0]["GroupId"]
    else:
        vpc = ec2.describe_vpcs(Filters=[{"Name": "isDefault", "Values": ["true"]}])["Vpcs"][0]
        group_id = ec2.create_security_group(GroupName=TAG, Description="Blueprint Postshot worker: no inbound, egress only", VpcId=vpc["VpcId"])["GroupId"]
    instance_type = arguments.instance_type or INSTANCE_TYPES[0]
    user_data = _render_user_data(run_id=arguments.run_id, staging=staging, urls=urls, instance_type=instance_type)
    try:
        run = ec2.run_instances(
            ...
            TagSpecifications=[
                {"ResourceType": "instance", "Tags": [{"Key": "Name", "Value": f"{TAG}-{arguments.run_id}"}, {"Key": RUN_TAG_KEY, "Value": arguments.run_id}, {"Key": "blueprint-ttl-minutes", "Value": str(ATTEMPT_5_TTL_MINUTES)}]},
                {"ResourceType": "volume", "Tags": [{"Key": RUN_TAG_KEY, "Value": arguments.run_id}, {"Key": "Name", "Value": f"{TAG}-{arguments.run_id}"}]},
            ],
        )
    except Exception:
        # Leave the external watchdog armed until the caller proves that no
        # allocation occurred; it is safer than silently cancelling the TTL.
        raise
    instance = run["Instances"][0]
    record = {
        "schema_version": "postshot_worker_launch.v2",
        "run_id": arguments.run_id,
        "attempt": 5,
        ...
        "phase_limits_seconds": PHASE_LIMITS_SECONDS,
        ...
        "expected_ebs_hourly_usd_upper_bound": EXPECTED_EBS_HOURLY_USD,
        "incremental_cap_usd": ATTEMPT_5_INCREMENTAL_CAP_USD,
        "instance_tag": {"key": RUN_TAG_KEY, "value": arguments.run_id},
        "raw_secret_values_recorded": False,
    }
    record["launch_digest"] = canonical_digest(record, digest_field="launch_digest")
    _write_artifact(state_dir / "launch.json", record)
    watchdog["instance_id"] = instance["InstanceId"]
    watchdog["record_digest"] = canonical_digest(watchdog, digest_field="record_digest")
    _write_artifact(state_dir / "external_watchdog.json", watchdog)
    print(json.dumps({key: record[key] for key in ("run_id", "attempt", "region", "instance_id", "instance_type", "ttl_deadline_utc", "incremental_cap_usd")}, indent=2))
