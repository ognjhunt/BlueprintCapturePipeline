#!/usr/bin/env python3
"""Render static scene video from ZeroScene bundle for DWM.

This script renders 3D Gaussian Splatting scenes from ZeroScene bundles
along the captured camera trajectory. The output is a static-scene video
suitable for use with DWM (Dexterous World Models).

Usage:
    # Basic usage
    python render_static_scene.py output/zeroscene -o static_scene.mp4

    # With options
    python render_static_scene.py output/zeroscene \
        --output static_scene.mp4 \
        --fps 30 \
        --backend cuda \
...
Examples:
    # Render using CPU (no GPU required)
    python render_static_scene.py zeroscene/ -o video.mp4 --backend cpu-numpy

    # Render with white background
    python render_static_scene.py zeroscene/ -o video.mp4 --background 1 1 1

    # Render at lower SH degree for speed
    python render_static_scene.py zeroscene/ -o video.mp4 --sh-degree 1
"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        choices=["diff-gaussian-rasterization", "gsplat", "cpu-numpy", None],
...
def list_backends() -> None:
    ...
    from blueprint_pipeline.video2zeroscene.rendering.rasterizer.backend_selector import (
        get_rasterizer_info
    )
