...
| `blueprint-audit-first-gpu-cross-repo-readiness --capture-repo <path> --webapp-repo <path> [--capture-root <path>]` | Cross-repo first owner-GPU E2E readiness audit | Inspects Capture/WebApp/Pipeline source contracts, optionally wraps the first-GPU capture-root readiness audit, checks the generated first-GPU launch order, GPU VM sync manifest, and VM runtime preflight plan, and writes `gpu_spend_decision` so blocked runs explicitly forbid RunPod/equivalent GPU allocation; writes an audit manifest only, does not call providers, submit WebApp requests, run simulators, provision GPUs, or upgrade proof |
...
| `blueprint-build-first-gpu-run-packet --capture-root <path> --webapp-site-slug <slug>` | Build first owner-GPU operator packet | Writes local/GPU command scripts, launch-order files, GPU VM runtime-preflight files, simulator path matrix files, provider bootstrap files, blocker-resolution files, scene-asset acquisition files, WebApp handoff files, and GPU VM sync checksum manifests under `pipeline/first_gpu_e2e_run_packet/`; does not download assets, copy files to a VM, provision GPUs, call providers, submit WebApp requests, run simulators, or upgrade proof |
