...
| `blueprint-audit-robot-eval-startup-architecture --job-dir <capture-root>/pipeline/robot_eval_jobs/<job_id>` | Read-only robot-eval startup architecture verifier | Reads existing job startup artifacts and writes `startup_architecture_audit.json`; checks async queueing, WebApp queue-only role, Pipeline scheduler ownership, CPU-preflight gating, prepared-worker/no-runtime-install contract, provider dry-run/no-secret envelope, RunPod adapter cost-control policy, redacted runtime/provider command artifacts, timeout/idle-shutdown controls, cost ledger, and proof ceilings; does not run providers, allocate GPUs, execute simulators, mutate secrets, or upgrade proof claims |
