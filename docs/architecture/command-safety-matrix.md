...
| `blueprint-audit-live-pipeline-setup --capture-root <path> --package-dir <dir> --digitalocean-droplet-name paperclip-prod-01 --digitalocean-droplet-ip 206.81.11.69` | Live setup/gate preflight | Loads local env files without printing secret values; checks gates, command hooks, SDK modules, WebApp upstream IDs, package audit status, and optional control-plane host; does not run provider jobs, mutate DigitalOcean, upload storage, or claim GPU/Arena proof |
