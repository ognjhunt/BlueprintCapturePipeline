...
| `blueprint-intake-live-pipeline-inputs --manifest-path <control-plane-manifest> --webapp-job-request <json> --arena-results-dir <dir> --policy-package <json> --real-robot-pov <json> --deployment-outcomes <json> --live-closure-evidence <json> [--stage-webapp-request] [--stage-arena-results] [--stage-policy-package] [--stage-real-robot-pov] [--stage-deployment-outcomes] [--stage-live-closure-evidence]` | Live external-input intake validator | Validates candidate WebApp request/envelope IDs against the configured capture root, inspects owner Arena result directories for JSON ingest artifacts, validates job-specific policy packages, real robot POV/action evidence, deployment outcomes, and closure evidence, optionally copies a validated WebApp request into the configured inbox, optionally stages Arena pointers, optionally copies real POV evidence to `pipeline/robot_eval_inputs/real_robot_pov_manifest.json`, and optionally copies policy/deployment/closure evidence under `pipeline/robot_eval_inputs/<job_id>/`; task/scenario outcome records can stage as real-world validation inputs, exact `scenario_eval_run_id` or `scenario_variation_instance_id` keys are required before predicted-vs-actual calibration is ready, real POV evidence must cover exact run/variation keys plus camera/action evidence refs, and proof remains blocked on `real_world_deployment_outcome_owner_evidence` and job-ingested real POV coverage until owner evidence exists; does not process the job, run Arena, execute policies, mutate env files, upload storage, or upgrade proof claims |
| `blueprint-live-pipeline-intake-service --host 127.0.0.1 --port 8765` | Authenticated WebApp-to-Pipeline intake service | Requires `BLUEPRINT_LIVE_PIPELINE_INTAKE_TOKEN`; accepts direct WebApp job requests, queue envelopes, robot-team policy packages, real robot POV manifests, deployment outcome JSON, and `live_robot_eval_closure_evidence.v1` bodies, stages validated requests/evidence through the same intake validator, and can optionally run a configured trigger command; does not run Arena, execute policies, call providers, publish claims, or set proof booleans |
| `blueprint-run-live-pipeline-control-plane --capture-root <path> --job-request-inbox <dir> --arena-results-dir <dir>` | Timer-safe live pipeline control-plane pass | Writes the control-plane manifest plus `live_pipeline_external_input_packet.json` and `.md`, drains queued WebApp job requests when configured, records exact missing WebApp IDs, owner Arena evidence, command hooks, SDK/operator gates, and generated real-world follow-up draft queues, and accepts WebApp inbox truth only when the queued request matches the configured capture root; exits 0 when blocked so timers do not restart-loop; does not auto-run follow-up reruns or run live simulator/provider/upload/operator paths unless separately gated |
...
| `BLUEPRINT_ALLOW_GPU_PROVIDER_LAUNCH=true blueprint-run-gpu-provider-launcher --job-dir <capture-root>/pipeline/robot_eval_jobs/<job_id> --allow-provider-launch` | Explicit GPU provider launcher handoff | Reads a prepared `gpu_provider_launch_request.json` and runs only the owner-supplied `BLUEPRINT_GPU_PROVIDER_LAUNCH_COMMAND` or `--provider-launch-command`; records `gpu_provider_launcher_result.json` plus stdout/stderr logs without raw command text or secret values and redacts known secret env values from captured logs; may trigger spend/provider side effects through that external command, but this module itself does not call provider APIs, prove allocation, run simulators, or upgrade robot/public proof |
| `blueprint-run-runpod-provider-adapter --provider-launch-request <job-dir>/gpu_provider_launch_request.json [--mode dry-run]` | RunPod provider adapter dry-run | Writes `runpod_provider_adapter_result.json` with RunPod serverless `/run` and GraphQL on-demand Pod request shapes plus the cost-control split between request policy, endpoint settings, and on-demand Pod watchdog requirements; performs no API call by default, stores no API key, and does not prove allocation, simulator execution, robot readiness, or public proof |
| `BLUEPRINT_ALLOW_RUNPOD_API_CALLS=true RUNPOD_API_KEY=<secret> blueprint-run-runpod-provider-adapter --provider-launch-request <job-dir>/gpu_provider_launch_request.json --mode <serverless-run|on-demand-pod> --allow-runpod-api-call` | Explicit RunPod API adapter call | Can submit a serverless `/run` job to an existing endpoint or create an on-demand Pod through RunPod GraphQL; may trigger spend/provider side effects, stores only redacted response/error data, and still does not prove simulator execution, robot readiness, safety, or public proof |
