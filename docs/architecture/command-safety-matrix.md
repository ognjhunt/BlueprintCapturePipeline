...
| `blueprint-audit-live-pipeline-proof-boundary --manifest-path <control-plane-manifest>` | Live control-plane proof-boundary audit | Reads the control-plane manifest, external input packet, and setup manifest; writes `live_pipeline_proof_boundary_audit.json`; fails on internal artifact inconsistency, secret leakage, or forbidden proof upgrades, but exits zero for a healthy waiting state with external blockers unless `--require-live-ready` is set |
