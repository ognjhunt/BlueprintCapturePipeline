...
| `blueprint-intake-live-pipeline-inputs --manifest-path <control-plane-manifest> --webapp-job-request <json> --arena-results-dir <dir> [--stage-webapp-request] [--stage-arena-results]` | Live external-input intake validator | Validates candidate WebApp request/envelope IDs against the configured capture root, inspects owner Arena result directories for JSON ingest artifacts, optionally copies a validated WebApp request into the configured inbox, and optionally writes `live_pipeline_staged_inputs.json` for the next control-plane pass; does not process the job, run Arena, mutate env files, upload storage, or upgrade proof claims |
| `blueprint-live-pipeline-intake-service --host 127.0.0.1 --port 8765` | Authenticated WebApp-to-Pipeline intake service | Requires `BLUEPRINT_LIVE_PIPELINE_INTAKE_TOKEN`; accepts direct WebApp job requests or queue envelopes, stages validated requests through the same intake validator, and can optionally run a configured trigger command; does not run Arena, call providers, publish claims, or set proof booleans |
