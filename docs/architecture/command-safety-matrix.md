...
| `blueprint-intake-live-pipeline-inputs --manifest-path <control-plane-manifest> --webapp-job-request <json> --arena-results-dir <dir> [--stage-webapp-request]` | Live external-input intake validator | Validates candidate WebApp request/envelope IDs against the configured capture root, inspects owner Arena result directories for JSON ingest artifacts, and optionally copies a validated WebApp request into the configured inbox; does not process the job, run Arena, mutate env files, upload storage, or upgrade proof claims |
