...
The next saved-input replay passed target matching and exposed a separate assumption: every background was required to be NuRec/ParticleField, while this owner-approved development scene contains an ordinary authored USD surface. The compiler now preserves ordinary USD geometry without conversion, measures its composed bounds, and names the same USD renderer in preflight/launch/policy receipts. Empty, point-cloud, volume and incorrect-unit assets cannot enter this branch. Bounds still have to contain required task positions; no collision/support check is weakened. This is development-only appearance, never a successful Marble-room registration or captured-scene fidelity claim.

The real packet next reached contact topology and refused a support Xform rather than its sole collision child. Scene-plan construction now resolves such a group only when exactly one collision body lies beneath it; multi-collider groups remain refused. Contact filtering retains the exact child path and unchanged collision bytes. This is format/name resolution, not a new supporting surface or a weakened contact requirement.
