...
## Relationship To The Perception Harness

`wam_derived_observation_bundle.json`,
`wam_derived_observation_manifest.json`,
`wam_perception_harness_checks.json`,
`wam_policy_observation_adapter_report.json`, and
`wam_derived_observation_steps.jsonl` are generated by the WAM-derived
perception/observation harness. The harness may also write
`wam_perception_harness_validation_report.json`,
`wam_false_success_reduction_metrics.json`, and
`wam_perception_harness_review_report.md`. Those artifacts can flag missing
targets, low-confidence tracks, relative-depth jumps, contact-likelihood
uncertainty, multiview generated-media disagreement, policy-adapter field
withholding, early termination, labeled validation-set accuracy, and
false-success reduction versus plain generated-video scoring. They are
diagnostic and gating support for the policy loop.

They do not prove forward/inverse consistency. The harness may include an
`inverse_action_consistency` field that points to the external scorer status, but
it must keep `forward_inverse_consistency_proven=false` unless this separate
episode-consistency scorer command actually ran and passed. Generated rollout
existence, generated-video success labels, mask/depth/contact-likelihood
outputs, validation metrics, review acceptance, and policy requery on a
harness-adapted observation are all separate from forward/inverse consistency
proof.

The sim-only provider E2E runner,
`python -m blueprint_pipeline.wam_sim_provider_e2e`, belongs to the same
separation. It can prove that a generated frame flows through SAM3/depth/pose
provider hooks, the WAM-derived harness, the policy observation adapter, and
claim-bounded scoring/requery gates. It still must not set
`forward_inverse_consistency_proven=true`; that field is controlled only by the
external episode-consistency scorer described below.
