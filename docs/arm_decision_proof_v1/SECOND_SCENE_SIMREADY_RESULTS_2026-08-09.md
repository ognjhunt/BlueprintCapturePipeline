...
## Hosted-model inventory and successor admission

The retained configurations and execution receipts show that the older model
was not isolated to Joint Agent. The effective construction stack was:

| Released agent | Effective hosted model in the retained run | Invocation status |
| --- | --- | --- |
| Joint Agent | `gpt-4.1` for `identify_asset.vlm`, `analyze_structure.llm`, `predict.vlm`, and `infer_articulation_candidates.vlm` | All four consumers ran in v24. |
| Material Agent | `gpt-4.1` VLM plus `gpt-4.1` parser/selection LLM | Both ran in Content Agents v5. |
| Physics Agent | `gpt-4.1` per-component prediction VLM | Prediction ran in v5; whole-asset identification was explicitly disabled. |
| Texture Agent | `gpt-image-1` | The one declared surface texture job ran in v5. |
| OVRTX, Scene Optimizer, USD validators, Gaussian excision, controls, scoring, and learned policies | No hosted construction model | These are renderer, geometry, deterministic validation, or frozen policy paths. |

The old hosted preflight equated a provider with one hard-coded model. That
contract is now replaced locally by an exact-model capability probe: a generated
16x16 primary-color PNG, strict JSON schema, response validation, token usage,
reasoning effort, and a canonical receipt. No scene bytes enter this probe.
`gpt-5.6-luna` with `reasoning_effort=max` reached the live Chat Completions API
but was rejected with HTTP 400, `unsupported_value` on `reasoning_effort`.
The same exact probe at `high` qualified with 64 input and 18 output tokens;
receipt `joint_agent/840796_luna_high_multimodal_structured_preflight.v2.json`,
digest `sha256:8eace0e349c86bdd550997476e4040d76692f8fafd32d7c874a7ccce783239e2`.
An `xhigh` probe also qualified with the same bounded token counts,
receipt digest
`sha256:987f1a2bf3ebf284f4d6a245f9e5efafc59dcbc4404e2401e0ead6bd523121bf`;
the successor execution setting is frozen to `xhigh`. The successful `high`
receipt remains a lower-effort comparison only. The unsuccessful max receipt is
retained beside them. The computed combined successful-probe cost is `$0.000344`
using the published Luna token rates; the rejected request reported no token
usage. The released NVIDIA v0.5.2 OpenAI backend uses Chat Completions, so this
qualification does not claim Responses API compatibility.

The successor defaults are now one shared harness contract, not three copied
scene settings. Every active Material, Physics, and Joint text/VLM node is
`gpt-5.6-luna` with `reasoning_effort=xhigh`; every active Texture image node is
`gpt-image-2`. The original 840313 fixture and the 840796 articulated fixture
are both pinned by the same hermetic test. Historical v5 receipts above retain
their actual GPT-4.1/GPT-Image-1 identities and are not rewritten.

Content Agents admission also no longer treats a successful model-catalog GET
as executable evidence. A bounded live GPT-Image-2 generation produced a
1024x1024 low-quality PNG (output digest
`sha256:77ac4cb849aa1b15503d0fcabf6d659fbf5848db4518add847b60e3353e1acc0`),
28 input and 196 output tokens, and canonical receipt
`joint_agent/840796_gpt_image_2_generation_preflight.v1.json`, digest
`sha256:b2221a3e3122740672b45dac649fae17f55c4610ad53d514187e252dc4db833a`.
The generated bytes were validated and discarded; no scene bytes were
uploaded. Its published 1024-square low-quality output estimate is `$0.006`.
Future bundle admission requires both this actual image-generation capability
shape and the Luna image-input plus strict-JSON capability shape. The reusable
migration landed and was pushed at `6f6d04744`.
...
spend including the Luna and GPT-Image-2 capability probes is `$6.081597`
against the `$12`
authority. Provider inventory was
