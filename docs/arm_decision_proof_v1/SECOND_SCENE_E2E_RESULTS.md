...
| Joint Agent v24, Vast 47323204 | All eight released steps completed; 28/28 component predictions and 11 articulation candidates were retained. Blueprint then guessed the wrong released optimizer output directory and could not perform deterministic bounds review; strict output-role discovery fix landed | USD 0.101801 exact | Destroyed; staged objects absent; provider zero |
| OpenAI Luna capability probes | `gpt-5.6-luna` `max` rejected before inference with typed `unsupported_value`; `high` and `xhigh` each passed a real image plus strict-JSON probe with 64 input/18 output tokens. The successor execution setting is frozen to `xhigh`. No scene bytes uploaded | USD 0.000344 computed estimate | No GPU/provider resource; canonical API receipts retained |
| OpenAI GPT-Image-2 capability probe | One actual 1024x1024 low-quality PNG generation passed; 28 input/196 output tokens; generated bytes validated then discarded; no scene bytes uploaded. Receipt digest `sha256:b2221a3e3122740672b45dac649fae17f55c4610ad53d514187e252dc4db833a` | USD 0.006 published output estimate | No GPU/provider resource; canonical API receipt retained |

Combined retained spend through Joint Agent v24 and the successor-model probes is **USD 6.081597**, below the USD 12 authority. Aura,
...
| `6f6d04744` | Freeze Luna `xhigh` and GPT-Image-2 successor defaults across Joint, Material, Physics, Texture, Responses labelers, and optional operators; replace catalog-only admission with bounded inference-backed text and image capability receipts. |

Every reusable fix has a focused hermetic regression. Before each commit, the
required selector and Ruff gate passed. Latest pre-document code gate: `1,082`
passed, `1` skipped, `9,367` deselected; Ruff passed over `src/` and `tests/`.
