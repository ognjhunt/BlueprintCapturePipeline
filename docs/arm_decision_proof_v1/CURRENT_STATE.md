...
| Item | Status | Observed evidence |
|---|---|---|
| ADP-009A | `partial` | The deterministic [`public_scene_suite_index.v1.json`](manifests/adp009a_materialized_suite/public_scene_suite_index.v1.json) now admits six of ten roles from observed bytes and execution receipts: selected InteriorGS scene `840313`, its exact SAGE-3D USDZ/collision companion, NVIDIA USD Content Agents v0.5.2, AuraFusion360's unchanged author-data smoke, the Blueprint-controlled native Isaac/PhysX positive control, and the exact project-owner-approved match-v2 SimReady can. Scene `840313` was selected under preregistered criteria with target `ins160` (`canned_beverage`) mapped to removable collider `/Root/ZHQYGJJVAJYEYPTUKY888888`; rejected candidate `841244` and shortlist failures remain retained. Aura `live_v14` executed the publisher workflow with unmodified source, produced an 820,832-vertex point cloud, retained eight `960x540` same-camera comparison pairs, used zero retries, cost an estimated `$0.788692`, and proved provider/object-store zero. That author smoke is not an InteriorGS result. The recovered positive-control files bind two deterministic Isaac Sim 6.0.1 rigid drop/contact cases, RTX pixels, the canonical Vast execution, teardown, and API-confirmed provider-zero receipts; they remain a synthetic development control. Exact SimReady run `run_011` binds the unchanged match-v2 asset and matching SAGE collision composition to native Isaac Sim 6.0.1 drop/contact, slide, tip, and gripper probes, all of which passed with zero retries, provider/object-store zero, and `$0.159906` estimated cost. The suite remains blocked on the Inpaint360GS author-data license authority, InFusion checkpoint license, controlled-background truth, and ScanNet++ transfer. |
...
blocked `6/10` replay opened and verified the referenced bytes and emitted index
receipt `sha256:54f7046710e49690bdb121a766c6b57bc0482ae9dc69c97ca8390e7e08923f1c`.
