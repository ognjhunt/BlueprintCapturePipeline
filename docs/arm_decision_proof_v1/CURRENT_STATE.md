...
| Item | Status | Observed evidence |
|---|---|---|
| Sim-only protocol | `observed_complete` | [`FOUNDER_SIM_ONLY_PROTOCOL.md`](FOUNDER_SIM_ONLY_PROTOCOL.md) freezes Isaac Lab-Arena's built-in Rubik's-cube-to-bowl task on Isaac Sim/PhysX, the DROID Franka-plus-Robotiq embodiment, explicit π0.5 baseline, GR00T N1.7-DROID alternative, 44 paired reset seeds per candidate, and an 88-episode schedule. Blueprint's founder approved exact protocol digest `sha256:3940d30009c4d9fdadac1ba4419eb2f04f8b22fd0273131f31d0112e1e2f6ad2`; the immutable [approval receipt](manifests/founder_sim_approval_receipt.json) and [execution admission](manifests/founder_sim_execution_admission.json) remain simulation-only and non-paid. [`adp_isaac_lab_arena_request.py`](../../src/blueprint_pipeline/adp_isaac_lab_arena_request.py) compiles its 88 frozen logical worker jobs without authorizing execution. |
...
| Production simulation | `missing` | Intentionally not started. Founder approval is complete. Checkpoint and Arena-asset materialization, native replay-or-scripted/zero-action and parity canaries, media-complete model-adapter dry runs, and a separately capped Vast admission remain prerequisites. |
