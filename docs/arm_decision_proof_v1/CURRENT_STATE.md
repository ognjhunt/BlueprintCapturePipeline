...
| Item | Status | Observed evidence |
|---|---|---|
| Sim-only protocol | `partial` | [`FOUNDER_SIM_ONLY_PROTOCOL.md`](FOUNDER_SIM_ONLY_PROTOCOL.md) freezes Isaac Lab-Arena's built-in Rubik's-cube-to-bowl task on Isaac Sim/PhysX, the DROID Franka-plus-Robotiq embodiment, explicit π0.5 baseline, GR00T N1.7-DROID alternative, 44 paired reset seeds per candidate, and an 88-episode schedule. Protocol digest `sha256:3940d30009c4d9fdadac1ba4419eb2f04f8b22fd0273131f31d0112e1e2f6ad2` is pending exact-digest founder approval; [`adp_isaac_lab_arena_request.py`](../../src/blueprint_pipeline/adp_isaac_lab_arena_request.py) compiles its 88 frozen logical worker jobs without authorizing execution. |
| GR00T adapter | `observed_complete` | [`groot_n17_droid_policy_runtime.py`](../../src/blueprint_pipeline/groot_n17_droid_policy_runtime.py) implements only the official NVIDIA ZMQ/DROID observation/action translation with pinned source and checkpoint revisions plus mandatory materialized-worker identity evidence. |
| Existing scene/assets | `partial` | Arena already registers the maple table, Rubik's cube, YCB bowl, home-office HDR, DROID embodiment, task, success metric, and OpenPI/GR00T remote-policy seams, so no generation or custom environment is required. Exact worker-side asset bytes still require a materialization receipt. |
...
| Production simulation | `missing` | Intentionally not started. Exact founder approval, checkpoint and Arena-asset materialization, native replay-or-scripted/zero-action and parity canaries, media-complete model-adapter dry runs, and capped paid-resource admission remain prerequisites. |
