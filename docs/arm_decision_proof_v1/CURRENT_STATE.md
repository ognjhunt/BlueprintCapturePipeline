...
| Item | Status | Observed evidence |
|---|---|---|
| ADP-009A | `partial` | The deterministic [`public_scene_suite_index.v1.json`](manifests/adp009a_materialized_suite/public_scene_suite_index.v1.json) now admits five of ten roles from observed bytes and execution receipts: selected InteriorGS scene `840313`, its exact SAGE-3D USDZ/collision companion, NVIDIA USD Content Agents v0.5.2, AuraFusion360's unchanged author-data smoke, and the Blueprint-controlled native Isaac/PhysX positive control. Scene `840313` was selected under preregistered criteria with target `ins160` (`canned_beverage`) mapped to removable collider `/Root/ZHQYGJJVAJYEYPTUKY888888`; rejected candidate `841244` and shortlist failures remain retained. Aura `live_v14` executed the publisher workflow with unmodified source, produced an 820,832-vertex point cloud, retained eight `960x540` same-camera comparison pairs, used zero retries, cost an estimated `$0.788692`, and proved provider/object-store zero. That author smoke is not an InteriorGS result. The recovered positive-control files bind two deterministic Isaac Sim 6.0.1 rigid drop/contact cases, RTX pixels, the canonical Vast execution, teardown, and API-confirmed provider-zero receipts; they remain a synthetic development control and do not qualify the exact can or public scene. Exact SimReady candidate run `vast_match_v2_native_run_007` adds native OVRTX and OVPhysX evidence, but the suite is still blocked on the Inpaint360GS author-data license authority, InFusion checkpoint license, controlled-background truth, native Isaac exact-SimReady qualification, and ScanNet++ transfer. |
...
not native OVRTX renders of the 3DGS background. OVPhysX proves candidate
drop/contact/settle behavior in its pinned runtime; it is not yet the required
Isaac Sim hybrid-scene qualification or physical truth.
...
blocked `5/10` replay opened and verified the referenced bytes and emitted index
receipt `sha256:ee89fe5989d997e52a5ba88a6da3f01c82708ae4ef2492e87d6013d0e61d4b7e`.
