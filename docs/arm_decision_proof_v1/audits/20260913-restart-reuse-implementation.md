...
- A terminal predecessor can recover through the existing authority/lineage checks. Terminal settlement does not erase unreconciled spend or executed-attempt counts. Proven never-queued dependent reservations can be released. Source or executed rows retain their conservative full reservation until cost reconciliation exists; no historical receipts are rewritten. Status no longer calls terminal-settled execution an unstarted cancellation.
...
These PRs do not deploy or restart services. Claude retains production ownership. The old host-local `/root/dedup_apply.py` is not changed by merging this repository; stop invoking that hard-link script. The maintained alternative, on a release containing this change, is:
...
The implementation is split into focused PRs to remain within the existing impacted-test limit: retained reuse, accounting/recovery, then workspace protection with both historical audit reports. All three are required for the complete repair set.
