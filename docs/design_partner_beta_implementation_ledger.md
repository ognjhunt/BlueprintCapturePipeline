...
| Pipeline live intake | Production remains independently pinned to `3bb376e7b987a34ee3fa0dc5e39c9ab42e6c59f8` with its existing rollback evidence. An isolated localhost-only staging checkout/service is active on port `8766` at protected-main commit `24d30264c50126b8a39ef69dd012d59893d9c054`, with a separate HMAC secret and state tree; exact version, unauthenticated denial, nonce replay denial, service stop/start rollback, and `systemd-analyze security` exposure `2.5 OK` passed while production remained active. Staging now allowlists the account-authorized host `f005.backblazeb2.com` and runs current `clamscan`; EICAR was rejected. A generated two-second MP4 traversed the real private Backblaze signed-download path into staging, received HTTP 200, exact SHA-256 `8551bbb55140249660ceedf987925e6017c050022a76daa150f46929badfce7b`, immutable admission, a clean malware result, and precise recapture instructions for duration and resolution. Its temporary B2 object version was deleted and the prefix verified empty. This proves deployed transfer/security/recapture behavior, not a rights-cleared real scan or large-file/WebApp staging integration. A dedicated `pipeline-handoff-listener` service account now has subscriber only on the exact subscription and objectViewer only on the exact capture bucket; the five stale project roles bound to the deleted `pipeline-runner` identity were removed and verification returns no remaining member. Enabling the prior timer exposed that its boot-relative first trigger had no next schedule on a long-running host. The committed PR #281 `OnActiveSec`/`OnUnitInactiveSec` candidate was installed with a rollback copy and then completed two independently scheduled authenticated pulls at `17:16` and `17:17` UTC; both returned success, acknowledged zero messages, and scheduled the next cycle. This proves recurring zero-message operation; it does not prove message processing. GCP billing is enabled; no paid workload was started. |
...
| Publication/deployment/parity | Integrated Pipeline, Capture, and WebApp slices published; Pipeline production and isolated staging identities proven separately | Pipeline PRs #279 and #280 published the isolated-staging intake and governance correction; protected main is `24d30264c50126b8a39ef69dd012d59893d9c054`, whose exact tree is deployed cleanly in isolated staging. Production intentionally remains the exact tested PR #275 runtime `3bb376e7b987a34ee3fa0dc5e39c9ab42e6c59f8` with its rollback backup. WebApp main and Render production remain exact `6e24b5e1dbba43e5f34f38221f353089525cb74d`; Capture remote main remains `c4e03f9dfc4d64e86aedc4a7c9905bfa7dd8e646`. Staging authentication, transfer, scanner, recapture, exact-version, isolation, and rollback checks pass. PR #281 contains the root-safe installer plus discovered Pub/Sub identity/cadence hardening and has not yet passed its successor hosted checks or merged. Physical-device Capture proof, a raw rights-cleared walkthrough, WebApp-to-staging large-upload proof, and the full real-capture vertical slice remain; neither production nor staging identity is physical/deployment-readiness evidence. |
