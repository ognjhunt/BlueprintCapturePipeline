...
| Authorized evidence execution | Partially completed | Router v1 remains hermetic and fail-closed. Explicitly allowlisted local analytic-reachability and captured-visibility adapters now exist; the registry is empty by default and unknown/live references fail closed. Traditional simulation, service-level authorization state, execution persistence, and customer-visible Decision Envelopes remain. |
