...
| Pipeline live intake | `https://paperclip.tryblueprint.io/api/live-pipeline/intake-audit` returns authenticated-route HTTP 401 without a token, proving the proxy route exists. The Paperclip host returns its SPA for `/health` and `/version.json`, so the exact service commit remains unverified. Read-only GCP inspection found the `blueprint-pipeline` Cloud Run job at generation 5 still references mutable `gcr.io/blueprint-8c1ca/blueprint-pipeline:latest`; its Ready condition is false with `ContainerPermissionDenied` (last transition `2026-07-02T20:04:41.555650Z`). Both Artifact Registry digest lookup and legacy image tag listing fail because billing is disabled. No token, IAM, billing, production configuration, or infrastructure was changed. |
...
| Publication/deployment/parity | Integrated Pipeline, Capture, and WebApp slices published | Pipeline PRs #248 and #249, Capture PR #60, and WebApp PR #429 are merged through protected main. Pipeline main is `97aa1721`; Capture main is `88c76130`; WebApp main and deployed production are `c0d8de74`. Local/remote tree and zero-divergence parity are proven in clean release checkouts; Capture's post-merge main CI is running. Pipeline production publication is not complete: the GCP job is unready, image identity is mutable/unverifiable, and correction would require billing/IAM or material infrastructure authorization. Staging/rollback proof for the full cross-repository service, physical-device Capture proof, and the real-capture vertical slice also remain. |
...
2. **Pipeline production identity/readiness:** the GCP `blueprint-pipeline` job
   ...
   image. Billing-disabled image APIs prevent digest attribution. Fixing billing,
   IAM, credentials, or material production infrastructure is outside the current
   authorization.
