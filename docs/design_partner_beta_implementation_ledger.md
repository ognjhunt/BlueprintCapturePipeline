...
| Reconstruction capability graph/result | Local control plane, external import, and WebApp adoption completed; further reconstruction methods remain | Versioned, provider-neutral method profiles/results and deterministic cheapest-sufficient set-cover planning are implemented. Explicitly authorized hermetic decoded-observation, strict ARKit V3.2 metric-scaffold, and source-bound PLY external-import adapters bind exact executors and immutable source bytes. The external-import adapter parses a size-bounded PLY header, verifies the exact asset and source-capture digests, and emits only an appearance layer with unknown coverage and no raw, captured-observation, metric, collision, physics, task, physical, deployment, safety, or ranking upgrade. Signed plan/authorize/execute/inspect endpoints are idempotent; WebApp displays the exact Pipeline-selected method and adapter, requires a distinct customer authorization, and cannot supply executors or provider choices. Arbitrary video cannot become calibration or metric authority. Appearance, metric/reference, semantic, and physics outputs remain separate. Local SfM/3DGS generation and independently qualified physics geometry remain. |
