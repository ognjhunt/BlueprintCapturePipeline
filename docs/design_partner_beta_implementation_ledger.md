...
| --- | --- | --- |
| BlueprintCapturePipeline | `21e49c3df1d4be7bacffda87bc9d78ce08e20bb4` | Startup primary checkout had a preserved user-owned `docs/CHANGELOG.md` edit and later supervisor work in a separate writer lane. The integrated controlled-beta work is published through PRs #248 and #249; the supervisor lane was independently published through PR #250. Protected `main` and `origin/main` are now `c54e581683d4268304c66b60a9df7f63596a6435`, tree `553b976b6f9529508113498aabc65b1069e25b79`. The primary checkout is clean and detached at that commit; this ledger and its deployment-identity follow-up remain isolated in a dedicated linked worktree. |
...
PR #250 subsequently published the agentic Task Evaluation Supervisor to
protected Pipeline `main` `c54e581683d4268304c66b60a9df7f63596a6435`,
tree `553b976b6f9529508113498aabc65b1069e25b79`; its CI, full CPU lane,
CodeQL, Python compatibility, and sim-only local-gate checks all passed. The
follow-on deployment-identity candidate is rebased on that exact main.
...
| Pipeline fast/full release lanes | PR #249's tree-identical final candidate `677f4ada508d72ec0fd607d9d2525c49c1ff0f11` passed `scripts/pytest_full.sh`: `7483 passed` in 1096.78s. Log SHA-256: `91f99e37fdc0bb85b934560f00955870f1d0624638e24fa97ea912e7ad625d74`. An earlier fast lane had `5816 passed` and one expected ledger-digest mismatch after the release evidence changed; the three exact ledger digests were refreshed and their focused verifier passed. It is not reported as an entirely green fast lane. All 16 PR checks and the post-merge CI, Full Test Lane, CodeQL, Python Compatibility, and Sim-Only Local Gate workflows passed on protected main. |
...
| Pipeline live intake | `https://paperclip.tryblueprint.io/api/live-pipeline/intake-audit` returns authenticated-route HTTP 401 without a token, proving the proxy route exists. The Paperclip host returns its SPA for `/health` and `/version.json`, so the exact service commit remains unverified. Read-only GCP inspection found the `blueprint-pipeline` Cloud Run job at generation 5 still references mutable `gcr.io/blueprint-8c1ca/blueprint-pipeline:latest`; its Ready condition is false with `ContainerPermissionDenied` (last transition `2026-07-02T20:04:41.555650Z`). The project owner enabled billing on `2026-07-30`, after which the current `latest` tag resolved to old image digest `sha256:b73a2a624c6acfa851353ea20c130f0f86a8b4665da5c2532f4916a873a5f168` with a `2026-03-16T15:21:50` tag timestamp. Billing is no longer a blocker, but the mutable, stale, unready deployment still cannot establish release parity. No token, IAM, production configuration, or infrastructure was changed by this implementation. |
...
| Pipeline deployment-identity seam | The follow-on publication candidate adds proxy-visible `/api/live-pipeline/version`. It returns HTTP 503 unless `BLUEPRINT_SOURCE_COMMIT` is an exact 40-hex commit; production Docker builds bind the checked-out `GIT_SHA`, and the response has the explicit claim ceiling `deployed_service_identity_only`. After rebasing on PR #250, the combined live-service, release-engineering, deployment-contract, and quality-ledger lane passes `98` tests; Ruff, shell syntax, and diff checks pass. This enables future parity proof but does not upgrade the currently unready deployment. |
...
| Real rights-cleared vertical slice | Exact external capture gate pending after safe local implementation | No rights-cleared physical capture is present in the three repositories, their current output roots, or the inspected user media folders. The repository walkthrough video is explicitly `repo_synthetic_fixture` with `fallback_allowed_for_beta_release=false`; the one non-fixture Pipeline MP4 is a generated `robot_pov.mp4`, not raw site evidence. Neither can satisfy the gate. Fixture-backed code, service, operator, and WebApp preparation is published; the exact minimal external input is documented below. Synthetic/hermetic tests remain tool-path evidence only. |
| Publication/deployment/parity | Integrated Pipeline, Capture, and WebApp slices published | Pipeline PRs #248, #249, and #250, Capture PR #60, and WebApp PR #429 are merged through protected main. Pipeline main is `c54e5816`; Capture main is `88c76130`; WebApp main and deployed production are `c0d8de74`. Local/remote tree and zero-divergence parity are proven in clean release checkouts, and Capture's post-merge main CI passed all six jobs. Pipeline production publication is not complete: billing is now enabled and the old image digest is attributable, but the GCP job is unready and still references a stale mutable tag. Staging/rollback proof for the full cross-repository service, physical-device Capture proof, and the real-capture vertical slice also remain. |
...
1. **Real capture:** no rights-cleared physical iPhone Pro/LiDAR or qualifying
   360 capture is locally available. The fixture matrix cannot satisfy this
   launch gate.
2. **Pipeline production identity/readiness:** the GCP `blueprint-pipeline` job
   is not Ready (`ContainerPermissionDenied`) and references a mutable `latest`
   image. Billing was enabled by the project owner on `2026-07-30`, so it is no
   longer a blocker and the old image digest is now attributable. Publishing a
   new immutable candidate still waits for the repository release gates, then
   established staging, rollback, IAM, and deployment checks; no production
   mutation is justified by billing alone.
