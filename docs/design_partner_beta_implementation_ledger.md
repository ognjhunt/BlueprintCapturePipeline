...
| Blueprint-WebApp | `92e4eacdcecb4b733b45998df8a8864bddebe2d4` | Primary checkout was clean and remains untouched. WebApp work continues in `codex/design-partner-beta-20260729`; secure resumable upload changes are committed at `5ff03f55`, and Pipeline-bound customer task review is committed at `b115089b`. |
...
| WebApp focused/build lanes | `14 passed` across capture-upload/task-review server, client transport, and component-DOM UI tests; `npm run check`, the exact Pipeline schema verifier, the graphify architecture pilot (44 files, 217 nodes, 309 edges), and `npm run build` passed against the worktree content subsequently committed at WebApp `b115089b`. The build ran immediately before the commit, so its embedded build-info SHA still names the prior commit `5ff03f55`; it is content buildability evidence, not the final exact-committed-SHA release gate. A Playwright journey was authored, but local browser execution is not counted: the host rejected loopback connections with `Can't assign requested address` even while the test server reported listening, and a retry encountered `EADDRINUSE`. The canonical WebApp final release suite has not run. |
...
| Task candidate discovery and approval | Pipeline contracts and WebApp review implemented; authoritative service handoff pending | `task_candidate_discovery.v1` separates observed facts, inferred affordances, unsupported regions, hazards, and privacy areas; all inferred candidates require digest-bound customer/operator approval. WebApp verifies the exact mirrored schema and canonical discovery/candidate digests, displays the separated evidence categories, and records owner approve/edit/reject/recapture commands atomically and idempotently. Transaction commit rechecks ownership and the live discovery binding; an exact replay can repair a missing session projection, while a discovery replacement fails closed. WebApp receipts remain `pending_pipeline_validation`, not approvals. Pipeline publication/command consumption and return of the authoritative decision, approved task, and Decision/Evidence Request remain. |
...
| WebApp state, task approval, artifacts | Partially completed | Secure 360/native/monocular upload sessions, progress, resume, history, digest-verified candidate review, approve/edit/reject/recapture controls, and proof-boundary copy are committed. Pipeline scientific-state handoff and command consumption, authoritative approval status, QA/recapture rendering, artifact download, and Decision Envelope completion remain. |
