...
The August 1 semantic continuation has now crossed two additional boundaries.
PR #282 publishes a deterministic standard-3DGS contribution
renderer that projects source-bound masks through retained-frame cameras into
front-to-back Gaussian weights. The next candidate implements a fail-closed
Meta SAM 3.1 Object Multiplex adapter for ordered hash-bound retained-frame JPEG
derivatives. It pins the official stateful API, code/runtime/checkpoint
identities, commercial-evidence and execution authorizations, offline inference,
persistent IDs, and untrusted tensor validation, then emits the existing source-
track provider result and ready import request. The adapter/import/contribution
regression is `29 passed`. No gated checkpoint, paid compute, or live GPU model
execution occurred, so this is executable integration code rather than accepted
semantic evidence.
...
| Repeatable public indoor proxy lane | `38 passed` across the new replay command, immutable Web-upload intake, reconstruction control plane, local external-import adapter, public claims, and readiness documentation; Ruff, diff checks, and all `72` public-claim surfaces pass. The command requires exact archive/PLY digests, rejects unsafe archive members and symlinked inputs, requires an explicit scanner-test-double acknowledgment, authorizes only the planned hermetic adapter, and asserts the frozen claim ceiling. The retained real MuSHRoom `koivu` archive replayed twice from clean commit `7e4445987973f58fbcda3852a5acde2684e8998b` with byte-identical summary SHA-256 `a4f9444a2047eaea6f7c9888ea73bdc2b9aba66320e7b066474ff9e055a4f191`. This is a real public observed-site proxy and not a customer upload, production security gate, raw iPhone/360 capture, metric/physics result, or physical proof. |
| Processed walkthrough Task Evaluation Run proxy | `104 passed` across processed-dataset testbed compilation, intake/QA, task discovery/control, local adapters, immutable compiler, and Router suites; Ruff passes. Exact clean commit `592f8da413d808dc707bf28c032559c30ac270f3` replayed the MuSHRoom `koivu` walkthrough twice into accepted processed-observation QA, an approval-required candidate, explicitly operator-approved proxy intent, immutable testbed `sha256:6675d53f...`, an explicitly authorized local visibility step, and partial Decision Envelope `sha256:1b7f430c...`. Processed visibility is supported; analytic reach, modeled collision, physical success, and comparative policy ranking abstain. The frozen verdict remains `thesis_not_supported`; cost is `$0`. The compact receipt is `docs/evidence/public_processed_task_testbed_mushroom_koivu_592f8da4.json`. This closes the public-dataset processed-observation/testbed/router proxy, not raw device capture, customer intent, metric/physics qualification, live production upload, or physical proof. |
...
| Pipeline live intake | Production remains independently pinned to `3bb376e7b987a34ee3fa0dc5e39c9ab42e6c59f8` with its existing rollback evidence. The isolated localhost-only staging checkout/service on port `8766` now runs protected-main commit `7063d7197dcc7d03a0a2ff6f56d1cf32433820e3`, whose tree `0354737d8657f813a3af9230b91f1dd434f731e3` exactly matches the fully tested PR #282 candidate. It has a separate HMAC secret and state tree; exact version, unauthenticated denial, nonce replay denial, service stop/start rollback, and `systemd-analyze security` exposure `2.5 OK` passed while production remained active. Staging allowlists the account-authorized host `f005.backblazeb2.com` and runs current `clamscan`; EICAR was rejected. A generated two-second MP4 traversed the real private Backblaze signed-download path into staging, received HTTP 200, exact SHA-256 `8551bbb55140249660ceedf987925e6017c050022a76daa150f46929badfce7b`, immutable admission, a clean malware result, and precise recapture instructions for duration and resolution. Its temporary B2 object version was deleted and the prefix verified empty. This proves deployed transfer/security/recapture behavior, not a rights-cleared real scan or large-file/WebApp staging integration. A dedicated `pipeline-handoff-listener` service account has subscriber only on the exact subscription and objectViewer only on the exact capture bucket; the five stale project roles bound to the deleted `pipeline-runner` identity were removed and verification returns no remaining member. The corrected `OnActiveSec`/`OnUnitInactiveSec` timer remains enabled and has completed recurring authenticated zero-message pulls. This proves recurring operation, not message processing. The current local Compute API response still says billing is disabled despite earlier enablement intent; no paid workload was started. |
...
| Reconstruction capability graph/result | Local control plane, external import, InteriorGS normalization, and WebApp adoption completed; further reconstruction/semantic methods remain | Versioned, provider-neutral method profiles/results and deterministic cheapest-sufficient set-cover planning are implemented. Explicitly authorized hermetic decoded-observation, strict ARKit V3.2 metric-scaffold, source-bound PLY external-import, and InteriorGS normalization paths bind exact executors and immutable source bytes. The generic external-import adapter emits only an appearance layer with unknown coverage and no raw, captured-observation, metric, collision, physics, task, physical, deployment, safety, or ranking upgrade. InteriorGS's supplied Z-up metric labels can be normalized as dataset annotations but remain bound by noncommercial terms and do not self-qualify physics. Signed plan/authorize/execute/inspect endpoints are idempotent; WebApp cannot supply executors or provider choices. Arbitrary video cannot become calibration or metric authority. Appearance, metric/reference, semantic, and physics outputs remain separate. The pure contribution-weighted semantic aggregation, bounded file-verification seam, candidate-only metric OBB fitter, compact provider-neutral source-track importer, deterministic small-scene/reference contribution renderer, and fail-closed official SAM 3.1 multiplex adapter are implemented. The SAM adapter requires exact retained-frame JPEG/PTS/camera evidence and explicit code/runtime/checkpoint/license/execution authority; it emits only inferred 2D source tracks. A separate digest-pinned, non-root GPU worker plus canonical Vast allocator/lifecycle now packages immutable inputs, fetches and hash-verifies the gated checkpoint with an ephemeral token, runs offline, validates bounded output, and proves teardown/provider zero. The gated model has not yet run: the live Vast account currently contains a separately owned policy-ranking instance, the local machine lacks safe image-build disk headroom, and MuSHRoom lacks truthful retained-video/encoder-retention evidence for this exact track contract. The renderer verifies exact source-track, retained-frame, camera, splat, and Gaussian-mapping digests; requires rectified OpenCV cameras; projects full anisotropic covariances; and emits front-to-back contribution weights without upgrading semantics, metric geometry, collision, or physics. Accelerated large-scene/chunked transport, accepted live detector execution, a reduced-authority processed-sequence semantic profile, local SfM/3DGS generation, support-plane/collision validation, and independently qualified physics geometry remain. |
...
| Real rights-cleared vertical slice | Public observed-site proxy runs through reconstruction; raw capture gate pending | MuSHRoom's CC BY 4.0 `koivu` iPhone room sequence is a rights-usable indoor site-walkthrough proxy. The official 146,575,749-byte archive matched publisher MD5 `a359dba714e7829be11747ce5dee141c` and local SHA-256 `68735cfa0758e1288a006c30dc8b95ffb4caa3392bc9c68c0c3ea6c111966518`; its 874 members had no traversal or links. The real 367,960-vertex binary PLY, SHA-256 `748af95d385bfedfb2058b28b59a6f431947c13deb34321719fcc6a2b16dac1e`, passed signed-transfer test-double ingestion, immutable admission, QA, reconstruction planning, explicit local authorization, execution, and compiler-input loading at zero paid-compute cost. `scripts/run_public_indoor_proxy.py` now makes this path repeatable from a clean commit with exact digest, archive-safety, adapter-authorization, claim-ceiling, and immutable-summary gates. Clean commit `7e4445987973f58fbcda3852a5acde2684e8998b` replayed the retained archive twice with byte-identical summary SHA-256 `a4f9444a2047eaea6f7c9888ea73bdc2b9aba66320e7b066474ff9e055a4f191`; the checked-in redacted receipt is `docs/evidence/public_indoor_proxy_mushroom_koivu_7e444598.json`. It correctly ended `partial` because decoded source observations were absent. Every metric/physics/physical/deployment/safety/ranking ceiling stayed false and `thesis_not_supported` stayed frozen. The transfer and malware scanner were test doubles, not deployed security proof. The sequence contains processed RGB/depth/poses and independent trajectories, but no original retained video, decoded PTS, IMU, tracking-reset log, or Raw Contract 3.2 encoder-retention evidence. It therefore proves a real observed-site proxy path, not the required raw iPhone/360 launch gate. The previously audited DataSnack household/human capture remains rejected. |
| Publication/deployment/parity | Integrated Pipeline, Capture, and WebApp slices published; Pipeline production and isolated staging identities proven separately | Pipeline PR #282 passed all hosted checks, including `6345 passed`, `8 skipped`, and `1667 deselected` in the fast lane plus `8020 passed` in the full lane. Rebase merge produced protected-main commit `7063d7197dcc7d03a0a2ff6f56d1cf32433820e3`; its tree `0354737d8657f813a3af9230b91f1dd434f731e3` exactly matches the tested candidate and is deployed cleanly in isolated staging with `0 0` divergence. Production intentionally remains the exact tested PR #275 runtime `3bb376e7b987a34ee3fa0dc5e39c9ab42e6c59f8` with its rollback backup. WebApp main and Render production remain exact `6e24b5e1dbba43e5f34f38221f353089525cb74d`; Capture remote main remains `c4e03f9dfc4d64e86aedc4a7c9905bfa7dd8e646`. Staging authentication, transfer, scanner, recapture, exact-version, isolation, rollback, and service-hardening checks pass. Physical-device Capture proof, a raw rights-cleared walkthrough, WebApp-to-staging large-upload proof, and the full real-capture vertical slice remain; neither production nor staging identity is physical/deployment-readiness evidence. |
...
1. **Raw real capture:** a rights-usable MuSHRoom indoor iPhone proxy is now
   ...
   that launch gate.
2. **Production promotion and recurring handoff:** billing is no longer a
   blocker. Isolated staging exact-version, authentication, signed transfer,
   scanner, targeted recapture, isolation, and rollback proofs pass. Production
   intentionally remains on the prior exact tested runtime. PR #281 must pass
   hosted checks and merge, the corrected timer must prove repeated zero-message
   pulls, and WebApp must target a real staging identity before promotion.
...
4. **Live configured service proof:** exact deployed HMAC, nonce replay denial,
   Backblaze host allowlist, ClamAV/EICAR behavior, immutable hashing, targeted
   recapture, object cleanup, and staging rollback now pass on a tiny generated
   transport fixture. Real resumable large-file WebApp transfer, external
   deletion receipts, and a rights-cleared launch capture remain unproven and
   must fail closed.
5. **Production semantic evidence adapters:** the deterministic contribution,
   ...
   importer contracts are implemented. The bounded standard-3DGS reference
   renderer now executes actual contribution compositing and produces a
   replayable lifting request. Accelerated large-scene contribution transport,
   live detector execution, and support/collision-scene adapters remain safe
   repo-local work. No
   semantic candidate is collision or physics truth without separately
   qualified evidence. The official SAM 3.1 worker image and fail-closed Vast
   lifecycle are implemented and hermetically tested but are not yet a live
   model result. MuSHRoom cannot be mislabeled as retained-video evidence; use a
   truthful reduced-authority processed-sequence profile or wait for the raw
   capture.
...
- Production Pipeline service identity was deployed to the exact tested runtime
  ...
  backup; isolated staging is separately bound to `24d30264c50126b8a39ef69dd012d59893d9c054`.
  An owner-authorized Backblaze transport fixture and least-privilege GCP
  listener credential/IAM change were performed. No customer capture, paid
  compute, live robot action, or physical-success claim was performed. No paid
  workload was started. Incremental provider cost incurred by this work: USD 0.
