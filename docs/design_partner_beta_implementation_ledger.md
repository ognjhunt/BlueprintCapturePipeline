...
| Immutable testbed compiler | Pipeline authority and WebApp v2 control completed hermetically; semantic projection release candidate in progress; deployed proof pending | The compiler generates immutable Site, Task, Scenario, and Eval Cards and binds exact evidence. Service submission v2 rejects caller-supplied intake, QA, reconstruction plan/results, semantic evidence, SimReady/placement conclusions, evaluator/reset artifacts, supported-condition claims, predecessor manifests, unknown fields, inconsistent robot bindings, caller-selected scientific scope, paid execution, live robot execution, and WebApp provider selection. The generated submission schema is mirrored exactly across repositories. Pipeline loads the exact reconstruction execution, derives conservative support artifacts, and writes them immutably. The current candidate additionally validates the full lifting -> metric OBB -> independent collision-consistency -> benchmark digest chain, capture/reconstruction/splat bindings, metric Z-up geometry, artifact references, and track-set equality before projecting semantic candidates. It never inserts those candidates into the physics layer and always records that they are not collision or physics qualification. WebApp submits only owner robot/decision inputs and requires exact signed publication before showing readiness. Automatic Pipeline-owned semantic bundle loading and successor-version owner control remain incomplete. |
...
5. **Production semantic evidence adapters:** the deterministic contribution,
   OBB, collision-consistency, benchmark, and testbed-projection contracts are
   implemented. A production contribution renderer, source-track importer,
   large-scene transport, support/collision-scene adapter, and automatic
   Pipeline-owned semantic bundle loader remain safe repo-local work. No
