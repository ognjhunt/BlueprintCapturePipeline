...
| --- | --- | --- |
| BlueprintCapturePipeline | `21e49c3df1d4be7bacffda87bc9d78ce08e20bb4` | Startup primary checkout had the preserved user-owned `docs/CHANGELOG.md` edit. The integrated controlled-beta work is now published through PRs #248 and #249; protected `main` is `97aa1721770ac6298414e24fa4e9b08f7891e131`. The primary checkout is intentionally still on `codex/task-evaluation-supervisor-phases-2-4` at `fa3c39ee` with user/other-writer edits to `docs/CHANGELOG.md` and `task_evaluation_supervisor/recovery.py`; publication and this ledger use clean linked worktrees and do not absorb those edits. |
...
PR #249. Current protected Pipeline `main` is
`97aa1721770ac6298414e24fa4e9b08f7891e131` with tree
`3a8f1fbb42eb8e4370df1120e4426829eea45323`. Its tree-identical local final
candidate `677f4ada508d72ec0fd607d9d2525c49c1ff0f11` passed the exact-final full suite
...
| Publication/deployment/parity | Integrated Pipeline, Capture, and WebApp slices published | Pipeline PRs #248 and #249, Capture PR #60, and WebApp PR #429 are merged through protected main. Pipeline main is `97aa1721`; Capture main is `88c76130`; WebApp main and deployed production are `c0d8de74`. Local/remote tree and zero-divergence parity are proven in clean release checkouts, and Capture's post-merge main CI passed all six jobs. Pipeline production publication is not complete: billing is now enabled and the old image digest is attributable, but the GCP job is unready and still references a stale mutable tag. Staging/rollback proof for the full cross-repository service, physical-device Capture proof, and the real-capture vertical slice also remain. |
