...
| Blueprint-WebApp | `92e4eacdcecb4b733b45998df8a8864bddebe2d4` | Primary checkout was clean and remains untouched. WebApp work continues in `codex/design-partner-beta-20260729`; secure resumable upload changes are committed at `5ff03f55`, customer task review at `b115089b`, the signed authoritative Pipeline approval handoff at `ce2abe2f`, and immutable testbed inspection at `e77f6d87`. |
...
| Authorized evidence execution | Partially completed | Router v1 remains hermetic and fail-closed. Explicitly allowlisted local analytic-reachability, captured-visibility, and swept-AABB collision-simulation adapters execute through a signed plan/authorize/execute/aggregate facade with append-only state and immutable results. The collision method requires qualified metric physics input and remains sim-only. Terminal artifacts publish to WebApp with exact receipt verification. Rich rigid-body/contact simulation and customer-facing authorization initiation remain. |
| WebApp state, task approval, artifacts | Partially completed | In addition to upload, authoritative task approval, and immutable testbed inspection, WebApp now accepts only a digest-valid native Pipeline Evidence Plan/Decision Envelope bound to the current testbed. History shows a compact run state; owner-only inspection renders per-claim decisions, partial decisions or abstention, unsupported conditions, disagreements, physical requests, next experiment, exact digests, and downloadable JSON without recomputing scientific truth. QA/recapture rendering, nonterminal planning/execution progress, customer authorization initiation, deployed URLs/tokens, and live network policy remain. |
