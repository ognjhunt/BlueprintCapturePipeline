...
| WebApp focused/build lanes | The task-approval handoff has `21 passed` across signed Pipeline publication, command forwarding, capture-upload/task-review server, client transport, and component-DOM UI tests. HMAC body binding, immutable replay, stale/raced discovery rejection, validly hashed but wrong response binding, proposer self-grading rejection, and retry-safe local persistence interruption are covered. `npm run check`, exact Pipeline schema parity (`cf4dcfc...` on both repos), Firestore/storage rule parity, graphify (44 files, 218 nodes, 310 edges), and `npm run build` passed during this phase. The build preceded the final response-binding hardening commit and embedded prior SHA `b115089b`; final exact-SHA build/release gates remain pending. A Playwright journey was authored, but local browser execution is not counted: the host rejected loopback connections with `Can't assign requested address` even while the test server reported listening, and a retry encountered `EADDRINUSE`. The canonical WebApp final release suite has not run. |
...
| WebApp state, task approval, artifacts | Partially completed | Secure 360/native/monocular upload sessions, progress, resume, history, digest-verified candidate review, approve/edit/reject/recapture controls, signed Pipeline discovery/decision handoff, and authoritative approved/rejected/recapture status are committed. QA/recapture rendering, testbed state, artifact download, and Decision Envelope completion remain. Deployed URLs/tokens/network policy are not configured or proven. |
