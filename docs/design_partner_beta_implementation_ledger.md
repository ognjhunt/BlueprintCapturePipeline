...
| Authorized evidence execution | Partially completed | Router v1 remains hermetic and fail-closed. Explicitly allowlisted local analytic-reachability and captured-visibility adapters execute through a signed plan/authorize/execute/aggregate facade with append-only state and immutable results. Terminal artifacts publish to WebApp with exact receipt verification. Traditional simulation and customer-facing authorization initiation remain. |
