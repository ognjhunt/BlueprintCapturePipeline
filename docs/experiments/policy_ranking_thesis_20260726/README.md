...
The first gate is the explicitly approved v2 pilot judge call. Its 8,192-token per-request ceiling is a maximum shared reasoning/output allowance, not expected usage; the 98-call upper bound is $8.2373725 and the exact retry cap is two. Results are resumable and use `store=false`. Only after all 98 predictions are persisted may pilot labels be joined. The one manually opened pilot session is logged and may not change gates.
