...
The v2 pilot judge call is complete. Its 8,192-token per-request ceiling was a maximum shared reasoning/output allowance, not expected usage; the completed run stayed below the $9.00 per-run cap, used `store=false`, and persisted all 98 predictions before any pilot label join. The one manually opened pilot session is logged and may not change gates. A first frozen pilot-label join then failed closed because the local RoboArena snapshot did not yield the required per-session/per-policy binary labels; `pilot_calibration_report.json` preserves all 49 missing-label blockers and no metric or ranking result. The next gate is resolving that attributable label-loading contract without changing predictions, thresholds, partitions, or the evaluator digest, then rerunning only the pilot join.
