...
| OmniPhotos `Unprocessed.zip` | University of Bath publishes original proprietary `.insv`, stitched MP4, frames, and SfM/configuration material for its scenes. The monolithic raw archive is 18,721,248,315 bytes, but ZIP64 range inspection found 31 `.insv` members. Blueprint extracted only the smallest member, `Nunobiki1/VID_20190404_152001_00_034.insv`, with archive CRC `fc2e0d85` and verified source SHA-256 `7ebf14367d8e41c09b483083ae0609c1f4c397aaad82b45f114a813d0a039c18`. | CC BY 4.0; local evaluation authorized. | Real proprietary-container probing and stitched/equirectangular PTS/DTS, split-isolation, and shared-center virtual-rig testing. | The tested member contains one already-equirectangular video stream, not raw dual-fisheye lens streams; it is outdoor, has no qualified physical trajectory or metric anchor, and cannot prove modern X4/X5 intake. | **Executed as a stitched 360 proxy.** It reached only the equirectangular virtual-rig claim ceiling and correctly abstained from native-rig, scale, reconstruction, collider, and Isaac claims. |
...
The University of Bath OmniPhotos ZIP64 archive was inspected by range without
downloading its 18.7 GB body. The smallest `.insv` member was extracted and
verified against its archive CRC. Blueprint's bounded native probe decoded 375
frames, retained 373 observed DTS values, preserved the two missing DTS values
as missing, and detected FFmpeg's `equirectangular` spherical-mapping side data.
That live probe exposed and drove fixes for two intake defects: stream side-data
values were previously omitted by the constrained probe, and per-frame DTS,
duration, and keyframe observations were digested but not retained.

The corrected run froze 16 actual-PTS observations into 11 training, two
validation, and three hidden held-out frames. Candidate and evaluator access
remained separate while compiling 156 and 36 deterministic shared-center
virtual views respectively; dataset and rig replay were exact. The source has
no raw dual-fisheye streams, physical camera trajectory, or metric anchor, so
the run ended `partial` at the equirectangular virtual-camera-rig ceiling. The
compact receipt is
...
- [University of Bath OmniPhotos dataset](https://researchdata.bath.ac.uk/948/)
