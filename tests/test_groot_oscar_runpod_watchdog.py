...
def test_owner_teardown_cancel_allows_one_unrelated_gpu_under_frozen_two_gpu_ceiling(
    tmp_path, monkeypatch
) -> None:
    now = 100.0
    deadline = 200.0
    ...
    receipt_path.write_text(
        json.dumps({"maximum_concurrent_paid_gpus_global": 2}),
        encoding="utf-8",
    )
    ...
    result = run_watchdog(
        ...
        provider_factory=lambda _name: Provider(),
        clock=lambda: now,
        sleeper=lambda _seconds: pytest.fail("scoped terminal state must not sleep"),
    )

    assert result["owner_teardown_cancel_requested"] is True
    assert result["maximum_concurrent_paid_gpus_global"] == 2
    assert result["global_inventory_below_prospective_ceiling"] is True
