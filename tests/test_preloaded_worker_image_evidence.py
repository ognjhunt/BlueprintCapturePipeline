...
def test_g4_self_test_converts_to_campaign_preload_evidence(tmp_path: Path):
    ...
    subprocess.run([
        sys.executable, "scripts/build_preloaded_worker_image_evidence.py",
        "--host-self-test", str(host), "--runtime-health", str(health),
        "--allocation-key", "blueprint-g4", "--host-image-id", "g4-host-1",
        "--output", str(output),
    ], check=True)
    evidence = json.loads(output.read_text())
    cfg = CampaignConfig(
        campaign_id="campaign-1", allocation_key="blueprint-g4",
        source_sha="5" * 40, image_digest="sha256:" + "7" * 64,
        hourly_rate_usd=4.5, max_provider_seconds=60,
        spend_authorization_usd=20, prior_exposure_usd=0,
...
def test_converter_rejects_failed_runtime_health(tmp_path: Path):
    host = tmp_path / "host.json"
    health = tmp_path / "health.json"
    host.write_text(json.dumps({"schema_version": "blueprint_g4_host_self_test.v1", "status": "passed"}))
    health.write_text(json.dumps({
        "schema_version": "groot_oscar_closed_loop_image_healthcheck.v1",
        "status": "blocked", "blockers": ["cuda_missing"],
    }))
