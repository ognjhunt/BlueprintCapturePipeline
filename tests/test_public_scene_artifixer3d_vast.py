...
def _prior_chain(tmp_path: Path) -> tuple[Path, Path]:
    ...
    prior["authorization_digest"] = canonical_digest(prior, digest_field="authorization_digest")
...
def test_bundle_kind_and_static_contract_are_registered(
    ...
    assert provider_runtime_contract_blockers(
        provider_bundle_kind="adp_artifixer3d",
        entrypoint_text=entrypoint.replace("artifixer3d_runner_failed_without_result", "removed"),
        runner_text=runner,
...
def test_paid_authority_chains_prior_spend_and_is_one_shot(
    ...
    assert (
        successor["prior_artifixer_attempt"]["authority_digest"]
        == authority["authorization_digest"]
    )
...
def test_dry_run_is_mutation_free(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    receipt_path, _ = _bundle(tmp_path, monkeypatch)
    bundle = validate_artifixer3d_bundle(receipt_path)
    import blueprint_pipeline.public_scene_artifixer3d_vast as subject
...
def test_container_registry_preflight_fails_before_paid_mutation(
    ...
    assert result["blockers"] == ["artifixer3d_container_image_not_registry_resolvable"]
...
def test_canonical_allocator_dry_run_validates_authority_without_consuming(
    ...
    assert admitted["status"] == "admitted"
    assert (
        admitted["allocation_binding"]["paid_attempt_authority_digest"]
        == authority["authorization_digest"]
    )
...
def test_canonical_allocator_dry_run_rejects_tampered_authority(
    ...
    assert blocked_admission["blockers"] == ["artifixer3d_paid_attempt_authority_invalid"]
