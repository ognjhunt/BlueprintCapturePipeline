...
@pytest.mark.parametrize("url", [
    "http://127.0.0.1:bad", "http://127.0.0.1:99999",
    "http://0.0.0.0:8443", "https://127.0.0.1:8443",
])
def test_controller_rejects_invalid_server_address_before_import(url: str) -> None:
    with pytest.raises(ValueError, match="g1_sonic_runtime_configuration_invalid"):
        assembly.build_pinned_g1_sonic_bridge(
            built=_built(), source_path=Path("/missing"),
            encoder_path=Path("/missing"), encoder_sha256="x",
            decoder_path=Path("/missing"), decoder_sha256="x", server_url=url,
        )
...
def test_supervised_episode_binds_scene_and_retains_teardown(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        assembly, "run_g1_built_scene_policy_episode",
    ...
    saved = json.loads((output_dir / "native_g1_supervised_built_scene_episode.v1.json").read_text())
    assert saved == result
