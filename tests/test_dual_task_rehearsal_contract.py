    ...
    return _seal(
        {
            ...
            "criterion_results": {
                criterion: True for criterion in REQUIRED_SELECTION_CRITERIA
            },
...
def task_freeze(task_id: str, task_kind: str, source_id: str) -> dict:
    ...
    payload = {
        ...
        "source_object": {
            ...
            "semantic_label": "washing_machine" if suffix == "a" else "stool",
            "observed_bounds_world_m": {
                "minimum": [0.0, 0.0, 0.0],
                "maximum": [0.6, 0.6, 0.9],
            },
            "support_or_attachment_id": "floor",
        },
        "removal_plan": {
...
def test_selection_scene_and_two_independent_task_freezes_validate() -> None:
    ...
    task_b = validate_task_freeze(
        task_freeze("task_b", "rigid_object_manipulation", "352")
    )
...
def test_join_rejects_shared_mask_collider_or_replacement() -> None:
    ...
    task_b = task_freeze("task_b", "rigid_object_manipulation", "352")
    for field in (
        "mask_set_id",
