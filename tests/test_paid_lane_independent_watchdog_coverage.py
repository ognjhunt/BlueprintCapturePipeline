...
def test_every_lane_that_arms_one_binds_it_to_the_names_it_creates() -> None:
    ...
    #: `adp_joint_agent_vast` is repaired on the branch that adds this file's
    #: sibling fix and its entry goes when that lands. `adp_aura_interiorgs_vast`
    #: is a retired appearance method with no launch profile, so its drift
    #: cannot reach a provider; it is recorded rather than repaired.
    known = {
        "adp_aura_interiorgs_vast",
        "adp_joint_agent_vast",
    }
