...
def _iteration_fixture(tmp_path: Path, status: str = "iteration") -> dict:
    fixture = production_fixture(tmp_path)
    provenance = {"schema_version": "blueprint.deploy_release_provenance.v1", "status": status,
    ...
    _write_json(fixture["release_provenance"], provenance)
    deploy = json.loads(fixture["deploy_receipt"].read_text())
    deploy["release_provenance"].update(provenance_status=status, promotion_eligible=False,
...
def test_iteration_release_requires_explicit_development_admission(tmp_path: Path, status: str) -> None:
    fixture = _iteration_fixture(tmp_path, status=status)
    with pytest.raises(SceneConfigurationSubmissionError, match="release_provenance_unproven"):
        _materialize(fixture)
    assert not fixture["staging_root"].exists()
    ...
    assert binding["promotion"] == {"workflow": None, "provenance_status": status,
...
@pytest.mark.parametrize("defect", ["unknown_status", "claim_upgrade", "numeric_false", "fake_run", "fake_workflow",
...
def test_iteration_admission_preserves_exact_release_and_truth_boundaries(tmp_path: Path, defect: str) -> None:
    ...
    if defect == "unknown_status":
        provenance["status"] = deploy["release_provenance"]["provenance_status"] = "untrusted"
