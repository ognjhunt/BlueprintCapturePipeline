...
def test_template_is_private_queue_worker_with_serverless_cache_mapping() -> None:
    ...
    assert payload["env"]["BLUEPRINT_GROOT_OSCAR_MODEL_CACHE"].startswith(
        "/runpod-volume/"
    )
...
def test_admission_blocks_unbounded_or_preexisting_compute() -> None:
    ...
    assert (
        "serverless_campaign_reservation_must_equal_remaining_wall_cap"
        in result["blockers"]
    )
...
def test_admission_blocks_rate_above_selected_gpu_ceiling() -> None:
    ...
    assert result["status"] == "blocked"
    assert (
        "serverless_campaign_hourly_rate_must_equal_l40s_ceiling"
        in result["blockers"]
    )
...
def test_inventory_joins_endpoints_templates_and_pods(monkeypatch) -> None:
    def fake_call(method, path, body, *, key, timeout):
        ...
        values = {
            "/endpoints": [
                {"name": "blueprint-groot-oscar-serverless-test-endpoint"}
            ],
            "/templates": [
                {"name": "blueprint-groot-oscar-serverless-test-template"}
            ],
            "/pods": [{"name": "unrelated"}],
        }
    ...
    monkeypatch.setattr(
        "blueprint_pipeline.groot_oscar_runpod_serverless._runpod_call", fake_call
    )
...
def test_active_worker_runs_all_phases_then_retrieves_and_tears_down(
    ...
    def fake_poll(*, operation_label, **_kwargs):
        outputs = {
            ...
            "kitchen-campaign": {
                ...
                "runs": [{"attempt_id": row} for row in ("smoke", "episode_001", "episode_002", "episode_003")],
    ...
    assert state["campaign_budget"]["max_hourly_rate_usd"] == 0.864
    assert isinstance(state["endpoint_allocated_at_epoch"], float)
    request_shapes = json.loads(
        (tmp_path / "out" / "serverless_request_shapes.json").read_text()
    )
    assert request_shapes["template"]["name"].startswith(
        "blueprint-groot-oscar-serverless-test-"
    )
    assert request_shapes["endpoint"]["name"].startswith(
        "blueprint-groot-oscar-serverless-test-"
    )
    ...
    failed_state = json.loads(
        (tmp_path / "endpoint-failed" / "watchdog_state.json").read_text()
    )
...
def test_model_volume_handoff_binding_requires_exact_volume_and_lane() -> None:
    ...
    assert serverless.validate_model_volume_handoff_binding(
        exact, volume_id="volume-1"
    ) == []
    assert set(
