...
def _semantic_receipts(
    ...
    *,
    ...
    paths: list[Path] = []
    source_receipt_path = source_root / "public_scene_artifixer3d_candidate_inputs.v3.json"
...
def test_binds_unreviewed_whole_frame_semantic_teacher_without_locality_claim(
    ...
    assert all(
        frame["whole_frame_candidate_preserved_without_compositing"] for frame in receipt["frames"]
    )
...
def test_materializes_same_pose_pairs_masks_review_path_and_unchanged_seed(
    ...
    assert (
        dual["shared_seed"]["retained_scene"]["sha256"] == source["shared_retained_scene"]["sha256"]
    )
    assert (
        dual["shared_seed"]["colmap_points3D"]["sha256"]
        == source["shared_colmap_initialization_points3D"]["sha256"]
    )
    ...
    transforms = json.loads(
        (task_root / task["transforms"]["relative_path"]).read_text(encoding="utf-8")
    )
    review = json.loads(
        (task_root / task["review_trajectory"]["relative_path"]).read_text(encoding="utf-8")
    )
    ...
    for frame, source_frame, teacher_frame in zip(
        ...
        original_path = source_task_root / source_frame["rendered_rgb"]["relative_path"]
        ...
        assert (
            ...
            == Path(teacher_frame["whole_frame_semantic_teacher"]["path"]).read_bytes()
        )
        mask = (
            np.asarray(
        ...
        actual_loss = np.asarray(
            Image.open(task_root / frame["anchor_loss_mask"]["relative_path"]).convert("L"),
            dtype=np.uint8,
        )
        axis = np.arange(-2, 3)
    ...
    assert dual["receipt_digest"] == canonical_digest(dual, digest_field="receipt_digest")
    assert (dual_root / "public_scene_artifixer3d_dual_target_inputs.v1.json").is_file()
...
def test_rejects_shape_camera_digest_radius_and_nonempty_output(
    ...
    for frame in task["frames"]:
        original = Image.open(task_root / frame["rendered_rgb"]["relative_path"]).convert("RGB")
    ...
    with pytest.raises(DualTargetInputError, match="semantic_teacher_camera_set_invalid"):
        materialize_dual_target_artifixer3d_inputs(
            source_candidate_inputs_receipt_path=(
                source_root / "public_scene_artifixer3d_candidate_inputs.v3.json"
            ),
