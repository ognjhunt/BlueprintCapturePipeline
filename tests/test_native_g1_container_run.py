...
def _inputs(root: Path) -> dict[str, Path]:
    ...
    request = {
        ...
        "sonic_provider_source": str(official / "isaaclab_twist2_g1/action_provider/action_provider_sonic.py"),
...
def test_container_plan_provisions_then_runs_same_sealed_worker(tmp_path: Path) -> None:
    ...
    assert shell.index("policy-runtime/bin/python") < shell.index("native_task_runtime_source_provision")
