...
def test_the_cli_can_supply_every_parameter_of_what_it_calls() -> None:
    """A receipt producer nothing can call is a lane that dead-ends.

    `materialize_broad_repair_support` had no entry point, on this branch or
    the one it was promoted from, which is what the materializer reachability
    budget caught. A flag table written by hand drops parameters silently, and
    `repair_support_dilation_pixels` selects *what gets built*, so fixing it at
    its default would fix a real decision invisibly. The required flags are
    therefore derived from the callee.
    """

    import inspect

    from blueprint_pipeline import public_scene_broad_repair_support as mod

    assert callable(getattr(mod, "main", None)), "materializer has no entry point"

    expected = {
        name
        for name, parameter in inspect.signature(
            mod.materialize_broad_repair_support
        ).parameters.items()
        if parameter.kind is inspect.Parameter.KEYWORD_ONLY
    }
    exposed = set(mod.CLI_PARAMETERS)

    assert exposed == expected, (
        f"CLI exposes {sorted(exposed)} but the callee takes {sorted(expected)}. "
        "Build the parser and the keyword arguments from one table so a "
        "parameter cannot be dropped."
    )
