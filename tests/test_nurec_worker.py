...
def test_nurec_worker_marker_includes_optional_scene_cleaning_artifacts(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    storage_root = tmp_path
    job_spec = tmp_path / "bucket/specs/nurec_job_spec.json"
    ...
    marker = tmp_path / "bucket/scenes/scene_1/captures/cap_1/pipeline/.nurec_complete"
    payload = json.loads(marker.read_text(encoding="utf-8"))
    outputs = payload.get("outputs", {})
    assert "inpainted_visual_mesh_glb" in outputs
    assert "sam3_instance_masks_dir" in outputs
    assert "colmap_undistorted_sparse_dir" in outputs
    assert "colmap_undistorted_images_dir" in outputs
