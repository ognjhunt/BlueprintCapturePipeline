...
def test_normalize_backend_name_accepts_loger() -> None:
    ...
    assert module._normalize_backend_name("NeoVerse") == module.BACKEND_NEOVERSE
    assert module._normalize_backend_name("GEN3C") == module.BACKEND_GEN3C
...
def test_run_reconstruction_compare_mode_supports_neoverse(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    module = _load_router_module()
    ...
    def _fake_neoverse(**kwargs):  # type: ignore[no-untyped-def]
        _write_backend_output(kwargs["output_dir"], primary_visual="export_last.usdz")
        (kwargs["output_dir"] / "neoverse_backend_report.json").write_text("{}", encoding="utf-8")
        return 0, "neoverse", ""

    monkeypatch.setattr(module, "_run_nurec_3dgrut", _fake_nurec)
    monkeypatch.setattr(module, "_run_neoverse", _fake_neoverse)

    report, winner = module.run_reconstruction(
        primary_backend=module.BACKEND_NUREC_3DGRUT,
        compare_backends=[module.BACKEND_NEOVERSE],
        compare_winner=module.BACKEND_NEOVERSE,
        job_spec_path=tmp_path / "job.json",
        input_video=tmp_path / "capture.mov",
        output_dir=tmp_path / "selected",
        scene_id="scene_demo",
    ...
    assert winner == module.BACKEND_NEOVERSE
    assert report["selected_winner"] == module.BACKEND_NEOVERSE
    assert module.BACKEND_NEOVERSE in report["runs"]
    assert (tmp_path / "selected" / "neoverse_backend_report.json").is_file()


def test_run_reconstruction_compare_mode_supports_gen3c(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    module = _load_router_module()

    def _fake_gen3c(**kwargs):  # type: ignore[no-untyped-def]
        _write_backend_output(kwargs["output_dir"], primary_visual="visual_mesh.glb")
        (kwargs["output_dir"] / "gen3c_backend_report.json").write_text("{}", encoding="utf-8")
        return 0, "gen3c", ""

    monkeypatch.setattr(module, "_run_gen3c", _fake_gen3c)

    report, winner = module.run_reconstruction(
        primary_backend=module.BACKEND_GEN3C,
        compare_backends=[],
        compare_winner="auto",
        job_spec_path=tmp_path / "job.json",
        input_video=tmp_path / "capture.mov",
    ...
    assert winner == module.BACKEND_GEN3C
    assert report["selected_winner"] == module.BACKEND_GEN3C
    assert (tmp_path / "selected" / "gen3c_backend_report.json").is_file()
