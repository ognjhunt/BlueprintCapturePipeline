...
def _one_bit_payload(rows: list[list[int]]) -> tuple[str, int, int]:
    """A real one_bit payload, produced by the parser's own encoder."""
    from meta_sam_parser._mask_codec import _encode_raster

    height, width = len(rows), len(rows[0])
    flat = [bit for row in rows for bit in row]
    return _encode_raster(flat, width, height), width, height


def test_decoding_many_masks_across_processes_matches_decoding_them_one_by_one(monkeypatch):
    """The pool is a scheduling choice; the rasters must not depend on it."""
    from blueprint_pipeline import meta_sam31

    masks = []
    for index in range(4):
        rows = [[1 if (x + y + index) % 3 else 0 for x in range(24)] for y in range(24)]
        payload, width, height = _one_bit_payload(rows)
        masks.append(meta_sam31._MaskPayload("one_bit", width, height, payload))

    serial = [meta_sam31._decode_mask_payload(mask) for mask in masks]
    assert meta_sam31._decode_masks(masks) == serial, "the floor should keep this serial"

    monkeypatch.setattr(meta_sam31, "PARALLEL_DECODE_PIXEL_FLOOR", 1)
    assert meta_sam31._decode_masks(masks) == serial


def test_a_pool_that_cannot_start_still_decodes_every_mask(monkeypatch):
    from concurrent import futures

    from blueprint_pipeline import meta_sam31

    rows = [[1 if x % 2 else 0 for x in range(16)] for _ in range(16)]
    payload, width, height = _one_bit_payload(rows)
    masks = [meta_sam31._MaskPayload("one_bit", width, height, payload)] * 3
    expected = [meta_sam31._decode_mask_payload(mask) for mask in masks]

    def refuse(*args, **kwargs):
        raise OSError("no workers available")

    monkeypatch.setattr(meta_sam31, "PARALLEL_DECODE_PIXEL_FLOOR", 1)
    monkeypatch.setattr(futures, "ProcessPoolExecutor", refuse)
    assert meta_sam31._decode_masks(masks) == expected
