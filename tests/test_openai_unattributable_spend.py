...
def test_reserves_the_full_cap_and_never_marks_cost_final(tmp_path: Path) -> None:
    ...
    assert receipt["strict_official_billing_satisfied"] is False
    assert receipt["terminal_billing_evidence"] is False
    assert receipt["billing_gap"] == "official_openai_cost_attribution_unavailable"
