...
def provider_execution_binding_blockers(
    ...
    *,
    ...
    minimal_diagnostic_failure = (
        diagnostic_only
        and execution.get("status") == "blocked_diagnostic_only"
        and not isinstance(chain, Mapping)
    )
    result_run_id = execution.get(
        "diagnostic_run_id" if diagnostic_only else "run_id"
    )
    if (
        (not minimal_diagnostic_failure or result_run_id is not None)
        and result_run_id != receipt.get("run_id")
    ) or (
    ...
    if (
        not minimal_diagnostic_failure or provider_source_commit is not None
    ) and provider_source_commit != receipt.get("source_commit"):
    ...
    if diagnostic_only and (
        not minimal_diagnostic_failure
        or execution.get("source_checkpoint_digest") is not None
    ) and execution.get("source_checkpoint_digest") != receipt.get(
        "source_diagnostic_checkpoint_digest"
    ):
        blockers.append("scene_configuration_diagnostic_checkpoint_mismatch")
    if diagnostic_only and (
        not minimal_diagnostic_failure
        or execution.get("diagnostic_bootstrap_mode") is not None
    ) and execution.get("diagnostic_bootstrap_mode") != receipt.get(
        "diagnostic_bootstrap_mode"
    ):
        blockers.append("scene_configuration_diagnostic_bootstrap_mode_mismatch")
    if diagnostic_only and not minimal_diagnostic_failure and (
        ...
        diagnostic_only
        and minimal_diagnostic_failure
        and execution.get("diagnostic_scientific_binding_digest") is not None
        and execution.get("diagnostic_scientific_binding_digest")
        != receipt.get("diagnostic_scientific_binding_digest")
    ):
        blockers.append("scene_configuration_diagnostic_scientific_binding_mismatch")
    return blockers
