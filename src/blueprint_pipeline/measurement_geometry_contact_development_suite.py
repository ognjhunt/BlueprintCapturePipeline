"""Plan or execute the checked MuJoCo rigid geometry/contact development corpus."""
...
SUITE_SCHEMA_VERSION = "capture_to_geometry_contact_development_suite.v1"
CORPUS_SCHEMA_VERSION = "capture_to_geometry_contact_development_corpus.v1"


class GeometryContactDevelopmentSuiteError(ValueError):
...
def _number(value: Any, code: str) -> float:
    if isinstance(value, bool):
        raise GeometryContactDevelopmentSuiteError(code)
    try:
        result = float(value)
    except (TypeError, ValueError) as exc:
        raise GeometryContactDevelopmentSuiteError(code) from exc
...
def _load_corpus(path: Path) -> dict[str, Any]:
    try:
        ...
        raise GeometryContactDevelopmentSuiteError(
            "geometry_contact_development_corpus_unreadable"
        ) from exc
    if not isinstance(value, Mapping):
        raise GeometryContactDevelopmentSuiteError("geometry_contact_development_corpus_not_object")
    corpus = dict(value)
    errors: list[str] = []
    if corpus.get("schema_version") != CORPUS_SCHEMA_VERSION:
        errors.append("geometry_contact_development_corpus_schema_invalid")
    if corpus.get("lane") != "rigid_contact":
        errors.append("geometry_contact_development_corpus_lane_invalid")
    for key, expected in (
        ...
        if corpus.get(key) is not expected:
            errors.append(f"geometry_contact_development_corpus_{key}_invalid")
    shared = corpus.get("shared_operating_point")
    cases = corpus.get("cases")
    if not isinstance(shared, Mapping):
        errors.append("geometry_contact_development_corpus_shared_point_invalid")
    if (
        not isinstance(cases, list)
        or len(cases) < 2
        or not all(isinstance(row, Mapping) for row in cases)
    ):
        errors.append("geometry_contact_development_corpus_cases_invalid")
    elif len({str(row.get("case_id", "")).strip() for row in cases}) != len(cases):
        errors.append("geometry_contact_development_corpus_case_ids_duplicate")
    if errors:
        raise GeometryContactDevelopmentSuiteError(*errors)
    return corpus


def run_capture_to_geometry_contact_development_suite(
    corpus_path: Path,
    *,
    ...
    """Run every public rigid-contact case through the uniform local boundary."""

    if not _valid_digest(qualification_split_digest):
        raise GeometryContactDevelopmentSuiteError(
            "geometry_contact_development_qualification_split_digest_invalid"
        )
    if not _valid_digest(controller_scope_digest):
        raise GeometryContactDevelopmentSuiteError(
            "geometry_contact_development_controller_scope_digest_invalid"
        )
    ...
    if qualification_split_digest == corpus_digest:
        raise GeometryContactDevelopmentSuiteError("geometry_contact_development_split_leakage")
    ...
    for index, raw_case in enumerate(corpus["cases"]):
        ...
        request = build_measurement_adapter_execution_request(
            ...
            spec,
            case,
            execution_id=f"mujoco-rigid-contact-{index + 1:03d}-{case_id}",
        ...
        bundle = run_measurement_adapter_execution(
            ...
            command_argv=[
                sys.executable,
                "-m",
                "blueprint_pipeline.measurement_mujoco_adapter",
            ],
            execute=execute,
        )
    ...
    if completed:
        penetration = [
            _number(
                ...
                "geometry_contact_development_penetration_metric_invalid",
            )
        ...
        contact_times = [
            _number(
                ...
                "geometry_contact_development_contact_time_invalid",
            )
            for row in summaries
        ]
        aggregate_metrics = {
    ...
    suite = {
        ...
        "lane": "rigid_contact",
        "solver_scope": "mujoco-rigid-drop-newton-implicitfast",
    ...
    return validate_capture_to_geometry_contact_development_suite(suite)


def validate_capture_to_geometry_contact_development_suite(
    value: Mapping[str, Any],
) -> dict[str, Any]:
    suite = json.loads(json.dumps(dict(value)))
    errors: list[str] = []
    if suite.get("schema_version") != SUITE_SCHEMA_VERSION:
        errors.append("geometry_contact_development_suite_schema_invalid")
    if suite.get("lane") != "rigid_contact":
        errors.append("geometry_contact_development_suite_lane_invalid")
    if suite.get("solver_scope") != "mujoco-rigid-drop-newton-implicitfast":
        errors.append("geometry_contact_development_suite_solver_scope_invalid")
    if suite.get("status") not in {
        "planned_not_executed",
        "completed_development_only",
        "incomplete",
    }:
        errors.append("geometry_contact_development_suite_status_invalid")
    cases = suite.get("cases")
    if not isinstance(cases, list) or len(cases) < 2:
        errors.append("geometry_contact_development_suite_cases_invalid")
    if suite.get("case_count") != len(cases or []):
        errors.append("geometry_contact_development_suite_case_count_mismatch")
    for key, expected in (
        ("development_only", True),
        ("synthetic_fixture", True),
        ("held_out", False),
        ...
        if suite.get(key) is not expected:
            errors.append(f"geometry_contact_development_suite_{key}_invalid")
    if suite.get("status") == "completed_development_only":
        if suite.get("all_cases_completed") is not True:
            errors.append("geometry_contact_development_suite_completion_invalid")
        if suite.get("all_replays_deterministic") is not True:
            errors.append("geometry_contact_development_suite_replay_invalid")
        if (
            not isinstance(suite.get("aggregate_metrics"), Mapping)
            or not suite["aggregate_metrics"]
        ):
            errors.append("geometry_contact_development_suite_metrics_missing")
    ...
    if suite.get("suite_digest") is not None and suite.get("suite_digest") != expected_digest:
        errors.append("geometry_contact_development_suite_digest_mismatch")
    if errors:
        raise GeometryContactDevelopmentSuiteError(*errors)
...
def main(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Plan or run the Capture-to-Geometry-and-Contact development corpus"
    )
    ...
    suite = run_capture_to_geometry_contact_development_suite(
        args.corpus,
