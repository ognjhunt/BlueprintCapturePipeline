...
from blueprint_pipeline.native_task_composition_diagnostic import (
    CompositionAdapters, CompositionDiagnosticError, file_record, run_composition_diagnostic,
    seal, validate_request, verify_record,
)
...
def _geometry_visibility_targets(stage, env_path, appearance_path):
    ...
    for prim in Usd.PrimRange(root, Usd.TraverseInstanceProxies()):
        ...
        while prim.IsInstanceProxy():
            prim = prim.GetParent()
        if any(child.HasAPI(UsdLux.LightAPI) for child in Usd.PrimRange(prim, Usd.TraverseInstanceProxies())):
...
def make_native_adapters(*, built, stage, request):
    ...
    appearance = next(row for row in built.plan['objects'] if row['semantic_role'] == 'scene_appearance')
    ...
    setting_names = ('/rtx/rendermode', '/omni/rtx/nre/compositing/rendererHints',
        '/rtx/rtpt/gaussian/skipTonemapping/enabled', '/renderer/multiGpu/enabled',
        '/UJITSO/geometry', '/rtx/post/aa/op', '/rtx/post/dlss/execMode',
    ...
    def snapshot():
        ...
        for path in paths:
            ...
            result[path] = {'authored': bool(spec and spec.HasInfo('default')),
    ...
    def apply(label):
        hidden = targets['native_meshes'] if label == 'appearance_only' else targets['appearance'] if label == 'native_meshes_only' else []
        with Usd.EditContext(stage, session_layer):
            for path in hidden:
    ...
    def fixed_state():
        ...
        for name in ['robot', *built.scene_asset_names.values()]:
            ...
            objects[name] = {field: _plain(getattr(data, field)) for field in ('root_pose_w', 'joint_pos') if hasattr(data, field)}
        return {'native_objects': objects, 'physics_time_seconds': float(env.sim.current_time),
            'physics_step_index': int(env.sim.current_time_step_index),
            'renderer_settings': {key: settings.get(key) for key in setting_names},
            'lights': {str(p.GetPath()): {a.GetName(): str(a.Get()) for a in p.GetAttributes()
                if a.GetName().startswith(('inputs:', 'xformOp', 'visibility'))} for p in light_prims},
            'camera': {'intrinsic_matrix': _plain(camera.data.intrinsic_matrices),
                'position_world_m': _plain(camera.data.pos_w), 'quaternion_world_opengl_xyzw': _plain(camera.data.quat_w_opengl)},
    ...
    def capture(label, output):
        ...
        native = camera.data
        data = SimpleNamespace(output={key: _array(value) for key, value in native.output.items()},
            info=native.info, intrinsic_matrices=_array(native.intrinsic_matrices),
            pos_w=_array(native.pos_w), quat_w_opengl=_array(native.quat_w_opengl))
        wrapped = SimpleNamespace(data=data, prim_path=getattr(camera, 'prim_path', None))
        row = _save_camera(output, request['camera_role'], wrapped, frame_index=0,
...
def main():
    ...
    app = env = None
    result = {'schema_version': 'native_task_arena_runtime_preflight.v1', 'status': 'blocked',
        'diagnostic_kind': 'fixed_native_mesh_particlefield_composition', 'preflight_only': True,
        'task_motion_executed': False, 'candidate_policy_queried': False, 'candidate_outcomes_accessed': False,
        'provider_zero_required_after_return': True, 'occlusion_qualified': False, 'blockers': []}
    try:
        from blueprint_pipeline.native_task_arena_construction_worker import _load_and_verify_manifest, preflight_native_dependency_matrix
        from blueprint_pipeline.native_task_arena_runtime import build_native_task_arena_environment
        from blueprint_pipeline.native_task_arena_preconstruction import prepare_native_task_arena_preconstruction
        from blueprint_pipeline.native_task_isaaclab_launch import launch_native_task_isaaclab, NATIVE_TASK_ARENA_DEVICE
        from blueprint_pipeline.native_task_nurec_render_setup import setup_and_warm_native_nurec_renderer
        ...
        request = validate_request(json.loads((runtime / 'runtime_inputs/composition_request.json').read_text()))
        plan_path = runtime / 'runtime_inputs/composition_scene_plan.json'
        verify_record(plan_path, request['resolved_scene_plan'])
        plan = json.loads(plan_path.read_text())
        if (plan['plan_digest'] != request['resolved_scene_plan_digest']
        ...
        result.update(request_digest=request['request_digest'], implementation_commit=manifest['implementation_commit'],
            bundle_input_digest=manifest['input_digest'], container_image=manifest['container_image'],
            source_packet_receipt_digest=manifest['packet_receipt_digest'])
        app, launch = launch_native_task_isaaclab(output / 'native_task_runtime_source_provisioning.v1.json',
            device=NATIVE_TASK_ARENA_DEVICE, appearance_render_path='particlefield_3d_gaussian_splat')
        ...
        warmup = setup_and_warm_native_nurec_renderer(app, stage, require_display_referred_particlefield=True)
        ...
        diagnostic = run_composition_diagnostic(request, output_root=output / 'composition', adapters=adapters)
