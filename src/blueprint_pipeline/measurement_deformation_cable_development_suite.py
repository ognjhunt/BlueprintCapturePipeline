"""Plan or execute the checked PyElastica cable development corpus."""
...
from .measurement_pyelastica_cable_adapter import (
    IMPLEMENTATION_ID,
...
SUITE_SCHEMA_VERSION = "capture_to_deformation_cable_development_suite.v1"
CORPUS_SCHEMA_VERSION = "capture_to_deformation_cable_development_corpus.v1"


class CableDevelopmentSuiteError(ValueError):
...
def _number(value: Any, code: str) -> float:
    if isinstance(value, bool):
        raise CableDevelopmentSuiteError(code)
    try:
        result = float(value)
    except (TypeError, ValueError) as exc:
        raise CableDevelopmentSuiteError(code) from exc
...
def _load_corpus(path: Path) -> dict[str, Any]:
    try:
        ...
        raise CableDevelopmentSuiteError("cable_development_corpus_unreadable") from exc
    if not isinstance(value, Mapping):
        raise CableDevelopmentSuiteError("cable_development_corpus_not_object")
    corpus = dict(value)
    errors: list[str] = []
    if corpus.get("schema_version") != CORPUS_SCHEMA_VERSION:
        errors.append("cable_development_corpus_schema_invalid")
    if corpus.get("lane") != "cable":
        errors.append("cable_development_corpus_lane_invalid")
    for key, expected in (
        ...
        if corpus.get(key) is not expected:
            errors.append(f"cable_development_corpus_{key}_invalid")
    shared = corpus.get("shared_operating_point")
    cases = corpus.get("cases")
    if not isinstance(shared, Mapping):
        errors.append("cable_development_corpus_shared_point_invalid")
    if (
        not isinstance(cases, list)
        or len(cases) < 2
        or not all(isinstance(row, Mapping) for row in cases)
    ):
        errors.append("cable_development_corpus_cases_invalid")
    elif len({str(row.get("case_id", "")).strip() for row in cases}) != len(cases):
        errors.append("cable_development_corpus_case_ids_duplicate")
    if errors:
        raise CableDevelopmentSuiteError(*errors)
    return corpus


def run_capture_to_deformation_cable_development_suite(
    corpus_path: Path,
    *,
    ...
    """Run all public cable cases through the uniform local adapter boundary."""

    if not _valid_digest(qualification_split_digest):
        raise CableDevelopmentSuiteError("cable_development_qualification_split_digest_invalid")
    if not _valid_digest(controller_scope_digest):
        raise CableDevelopmentSuiteError("cable_development_controller_scope_digest_invalid")
    ...
    if qualification_split_digest == corpus_digest:
        raise CableDevelopmentSuiteError("cable_development_split_leakage")
    ...
    for index, raw_case in enumerate(corpus["cases"]):
        ...
        request = build_measurement_adapter_execution_request(
            ...
            spec,
            case,
            execution_id=f"pyelastica-cable-{index + 1:03d}-{case_id}",
        ...
        bundle = run_measurement_adapter_execution(
            ...
            command_argv=[
                sys.executable,
                "-m",
                "blueprint_pipeline.measurement_pyelastica_cable_adapter",
            ],
            execute=execute,
        )
        ...
        summaries.append(
            {
                ...
                "maximum_segment_strain": runtime.get("maximum_segment_strain"),
                "observed_metrics": (
    ...
    if completed:
        displacement = [
            _number(
                ...
                "cable_development_state_trajectory_metric_invalid",
            )
            for row in summaries
        ]
        force = [
            _number(
                row["observed_metrics"]["force"],
                "cable_development_force_metric_invalid",
            )
            for row in summaries
        ]
        strain = [
            _number(
                row["maximum_segment_strain"],
                "cable_development_strain_metric_invalid",
            )
            for row in summaries
        ]
        aggregate_metrics = {
            "mean_tip_displacement_m": fmean(displacement),
            "maximum_tip_displacement_m": max(displacement),
            "mean_applied_gravity_force_n": fmean(force),
            "maximum_segment_strain": max(strain),
        }
    suite = {
    ...
    return validate_capture_to_deformation_cable_development_suite(suite)


def validate_capture_to_deformation_cable_development_suite(
    value: Mapping[str, Any],
) -> dict[str, Any]:
    suite = json.loads(json.dumps(dict(value)))
    errors: list[str] = []
    if suite.get("schema_version") != SUITE_SCHEMA_VERSION:
        errors.append("cable_development_suite_schema_invalid")
    if suite.get("lane") != "cable":
        errors.append("cable_development_suite_lane_invalid")
    if suite.get("status") not in {
        "planned_not_executed",
        "completed_development_only",
        "incomplete",
    }:
        errors.append("cable_development_suite_status_invalid")
    cases = suite.get("cases")
    if not isinstance(cases, list) or len(cases) < 2:
        errors.append("cable_development_suite_cases_invalid")
    if suite.get("case_count") != len(cases or []):
        errors.append("cable_development_suite_case_count_mismatch")
    for key, expected in (
        ("development_only", True),
        ("synthetic_fixture", True),
        ("held_out", False),
    ...
    if suite.get("suite_digest") is not None and suite.get("suite_digest") != expected_digest:
        errors.append("cable_development_suite_digest_mismatch")
    if errors:
        raise CableDevelopmentSuiteError(*errors)
...
def main(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Plan or run the Capture-to-Deformation cable development corpus"
    )
    ...
    suite = run_capture_to_deformation_cable_development_suite(
        args.corpus,
