"""Plan or execute the checked MuJoCo peg-insertion development corpus."""
...
from .measurement_mujoco_insertion_adapter import (
    IMPLEMENTATION_ID,
...
SUITE_SCHEMA_VERSION = "capture_to_geometry_contact_insertion_development_suite.v1"
CORPUS_SCHEMA_VERSION = "capture_to_geometry_contact_insertion_development_corpus.v1"


class InsertionDevelopmentSuiteError(ValueError):
...
def _number(value: Any, code: str) -> float:
    if isinstance(value, bool):
        raise InsertionDevelopmentSuiteError(code)
    try:
        result = float(value)
    except (TypeError, ValueError) as exc:
        raise InsertionDevelopmentSuiteError(code) from exc
...
def _load_corpus(path: Path) -> dict[str, Any]:
    try:
        ...
        raise InsertionDevelopmentSuiteError("insertion_development_corpus_unreadable") from exc
    if not isinstance(value, Mapping):
        raise InsertionDevelopmentSuiteError("insertion_development_corpus_not_object")
    corpus = dict(value)
    errors: list[str] = []
    if corpus.get("schema_version") != CORPUS_SCHEMA_VERSION:
        errors.append("insertion_development_corpus_schema_invalid")
    if corpus.get("lane") != "peg_insertion":
        errors.append("insertion_development_corpus_lane_invalid")
    for key, expected in (
        ...
        if corpus.get(key) is not expected:
            errors.append(f"insertion_development_corpus_{key}_invalid")
    ...
    if (
        ...
        errors.append("insertion_development_corpus_cases_invalid")
    elif len({str(row.get("case_id", "")).strip() for row in cases}) != len(cases):
        errors.append("insertion_development_corpus_case_ids_duplicate")
    if not isinstance(corpus.get("shared_operating_point"), Mapping):
        errors.append("insertion_development_corpus_shared_point_invalid")
    if errors:
        raise InsertionDevelopmentSuiteError(*errors)
    return corpus


def run_capture_to_geometry_contact_insertion_development_suite(
    corpus_path: Path,
    *,
    ...
    if not _valid_digest(qualification_split_digest):
        raise InsertionDevelopmentSuiteError(
            "insertion_development_qualification_split_digest_invalid"
        )
    if not _valid_digest(controller_scope_digest):
        raise InsertionDevelopmentSuiteError(
            "insertion_development_controller_scope_digest_invalid"
        )
    ...
    if qualification_split_digest == corpus_digest:
        raise InsertionDevelopmentSuiteError("insertion_development_split_leakage")
    ...
    for index, raw_case in enumerate(corpus["cases"]):
        ...
        request = build_measurement_adapter_execution_request(
            ...
            spec,
            case,
            execution_id=f"mujoco-insertion-{index + 1:03d}-{case_id}",
        ...
        bundle = run_measurement_adapter_execution(
            ...
            command_argv=[
                sys.executable,
                "-m",
                "blueprint_pipeline.measurement_mujoco_insertion_adapter",
            ],
            execute=execute,
        )
        ...
        summaries.append(
            {
                "case_id": case_id,
                "lateral_offset_m": operating_point["lateral_offset_m"],
                ...
                "final_insertion_depth_m": runtime.get("final_insertion_depth_m"),
                "maximum_contact_count": runtime.get("maximum_contact_count"),
                "observed_metrics": (
    ...
    if completed:
        penetration = [
            _number(
                ...
                "insertion_development_penetration_invalid",
            )
            for row in summaries
        ]
        clearance = [
            _number(
                row["observed_metrics"]["minimum_clearance_error"],
                "insertion_development_clearance_invalid",
            )
            for row in summaries
        ]
        aggregate_metrics = {
            "successful_insertion_case_count": sum(
                row["observed_metrics"]["insertion_success_boundary"] is True for row in summaries
            ),
            "unsafe_case_count": sum(
                row["unsafe_condition_predicted"] is True for row in summaries
            ),
            "maximum_penetration_m": max(penetration),
            "minimum_signed_clearance_m": min(clearance),
            "mean_signed_clearance_m": fmean(clearance),
        }
    suite = {
        ...
        "lane": "peg_insertion",
        "solver_scope": "mujoco-square-peg-newton-implicitfast",
    ...
    return validate_capture_to_geometry_contact_insertion_development_suite(suite)


def validate_capture_to_geometry_contact_insertion_development_suite(
    value: Mapping[str, Any],
) -> dict[str, Any]:
    suite = json.loads(json.dumps(dict(value)))
    errors: list[str] = []
    if suite.get("schema_version") != SUITE_SCHEMA_VERSION:
        errors.append("insertion_development_suite_schema_invalid")
    if suite.get("lane") != "peg_insertion":
        errors.append("insertion_development_suite_lane_invalid")
    if suite.get("solver_scope") != "mujoco-square-peg-newton-implicitfast":
        errors.append("insertion_development_suite_solver_scope_invalid")
    if suite.get("status") not in {
        "planned_not_executed",
        "completed_development_only",
        "incomplete",
    }:
        errors.append("insertion_development_suite_status_invalid")
    cases = suite.get("cases")
    if not isinstance(cases, list) or len(cases) < 2:
        errors.append("insertion_development_suite_cases_invalid")
    if suite.get("case_count") != len(cases or []):
        errors.append("insertion_development_suite_case_count_mismatch")
    for key, expected in (
        ("development_only", True),
        ("synthetic_fixture", True),
        ("held_out", False),
        ...
        if suite.get(key) is not expected:
            errors.append(f"insertion_development_suite_{key}_invalid")
    if suite.get("status") == "completed_development_only":
        if suite.get("all_cases_completed") is not True:
            errors.append("insertion_development_suite_completion_invalid")
        if suite.get("all_replays_deterministic") is not True:
            errors.append("insertion_development_suite_replay_invalid")
        if (
            not isinstance(suite.get("aggregate_metrics"), Mapping)
            or not suite["aggregate_metrics"]
        ):
            errors.append("insertion_development_suite_metrics_missing")
    ...
    if suite.get("suite_digest") is not None and suite.get("suite_digest") != expected_digest:
        errors.append("insertion_development_suite_digest_mismatch")
    if errors:
        raise InsertionDevelopmentSuiteError(*errors)
...
def main(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Plan or run the MuJoCo peg-insertion development corpus"
    )
    ...
    suite = run_capture_to_geometry_contact_insertion_development_suite(
        args.corpus,
