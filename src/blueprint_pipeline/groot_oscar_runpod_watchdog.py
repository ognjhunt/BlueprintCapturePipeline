        ...
        if cancel_candidate:
            ...
            try:
                ...
                if resolved_provider == "vast":
                    ...
                    if recorded_vast_instance.get("status") == "not_recorded":
                        # No create response ever yielded an instance id. Two
                        # independent exact-prefix plus global zero inventories
                        # below are then the complete provider-side absence proof.
                        exact_contract_zero = recorded_vast_instance.get("required") is False
                    else:
