...
SEMANTIC_TEACHER_SCHEMA = (
    "public_scene_whole_frame_semantic_teacher_candidates.v1"
)
...
def _validated_source(path: Path) -> dict[str, Any]:
    ...
    if (
        ...
        != "candidate_inputs_prepared_no_model_no_execution"
...
def _task_root(task: Mapping[str, Any]) -> Path:
    return _directory(
        task.get("scene_directory"), code="dual_target_source_task_root_invalid"
    )
...
def _source_task_frames(task: Mapping[str, Any]) -> list[dict[str, Any]]:
    ...
    for expected_index, frame in enumerate(frames):
        ...
        rgb = _image(
            original, mode="RGB", code="dual_target_source_original_invalid"
        )
...
def _validated_transforms(
    ...
    path = _bound_record(
        task.get("transforms"), code="dual_target_source_transforms_invalid"
    )
...
def _validated_semantic_teacher(
    ...
    bound_source = _bound_record(
        source_record, code="dual_target_semantic_teacher_receipt_invalid"
    )
    if (
...
def _teacher_frame_map(receipt: Mapping[str, Any]) -> dict[str, dict[str, Any]]:
    ...
    for frame in receipt.get("frames") or []:
        ...
        for field in (
            ...
            _bound_record(
                frame.get(field), code="dual_target_semantic_teacher_frame_invalid"
            )
...
def _write_colmap_calibration(
    sparse: Path, transform_rows: Sequence[Mapping[str, Any]]
) -> None:
    with (sparse / "cameras.bin").open("wb") as cameras, (
        sparse / "images.bin"
    ).open("wb") as images:
        ...
        for image_id, row in enumerate(transform_rows, start=1):
            ...
            camera_to_world_opengl = np.asarray(
                row["transform_matrix"], dtype=np.float64
            )
            camera_to_world_opencv = (
                camera_to_world_opengl @ CAMERA_CONVENTION_FLIP
            )
        ...
        rows.append(
            {
                ...
                "inside_exact_support_changed_pixels": int(
                    np.count_nonzero(changed & support)
                ),
                "outside_exact_support_changed_pixels": int(
                    np.count_nonzero(changed & ~support)
                ),
...
def materialize_dual_target_artifixer3d_inputs(
    *,
    ...
    for unresolved in semantic_teacher_receipt_paths:
        path = _file(
            unresolved, code="dual_target_semantic_teacher_receipt_missing"
        )
        receipt = _validated_semantic_teacher(
            path, source_path=source_path, source=source
        )
        task_id = str(receipt.get("task_id") or "")
        if not task_id or task_id in semantic_receipts:
            raise DualTargetInputError(
                ["dual_target_semantic_teacher_task_set_invalid"]
            )
    ...
    for task_id in task_ids:
        ...
        source_frames = _source_task_frames(task)
        source_transforms, _source_transforms_path = _validated_transforms(
            task, source_frames
        )
        teacher_receipt = semantic_receipts[task_id]
        teacher_by_camera = _teacher_frame_map(teacher_receipt)
        if set(teacher_by_camera) != {
            str(frame["camera_id"]) for frame in source_frames
        }:
            raise DualTargetInputError(
                ["dual_target_semantic_teacher_camera_set_invalid"]
            )
        copied_teacher_receipt = (
            provenance / f"semantic_teacher.{task_id}.json"
        )
        _link_or_copy(
        ...
        for physical_index, (source_frame, transform) in enumerate(
            ...
            if (
                ...
                raise DualTargetInputError(
                    ["dual_target_semantic_teacher_binding_invalid"]
                )
            exact_support = source_frame["mask"] > 0
            teacher_changed = np.any(
                teacher_pixels != source_frame["rgb"], axis=2
            )
            if (
            ...
            frame_rows.append(
                {
                    ...
                    "source_original_frame": _absolute_record(
                        source_frame["original_path"]
                    ),
                    "source_exact_repair_mask": _absolute_record(
                        source_frame["mask_path"]
                    ),
                    "source_whole_frame_semantic_teacher": _absolute_record(
                        teacher_source
                    ),
                    "anchor_rgb": _relative_record(anchor_path, root=task_root),
                    "semantic_teacher_rgb": _relative_record(
                        teacher_path, root=task_root
                    ),
                    "semantic_teacher_override_rgb": _relative_record(
                        teacher_override_path, root=task_root
                    ),
                    "exact_repair_mask": _relative_record(
                        exact_mask_path, root=task_root
                    ),
                    "anchor_loss_mask": _relative_record(
                        anchor_mask_path, root=task_root
                    ),
        ...
        top_intrinsics = {
            key: value
            for key, value in source_transforms.items()
            if key != "frames"
        }
            ...
            "frames": [
                {
                    ...
                    "semantic_teacher_training_index": row[
                        "semantic_teacher_training_index"
                    ],
                }
        ...
        task_receipts.append(
            {
                ...
                "selected_anchor_indices_file": _relative_record(
                    selected_path, root=task_root
                ),
                ...
                "review_trajectory": _relative_record(
                    review_path, root=task_root
                ),
                "camera_index": {
                ...
                "source_colmap": {
                    "cameras": _relative_record(
                        sparse / "cameras.bin", root=task_root
                    ),
                    "images": _relative_record(
                        sparse / "images.bin", root=task_root
                    ),
                    "points3D": _relative_record(
                        sparse / "points3D.bin", root=task_root
                    ),
                },
                "loss_contract": {
                    "original_anchor": (
