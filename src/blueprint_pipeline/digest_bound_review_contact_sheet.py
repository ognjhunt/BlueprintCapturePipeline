...
def _load_frames(*, result_path: Path, rows: Sequence[Mapping[str, Any]]) -> list[dict[str, Any]]:
        ...
        placements.append(
            {
                ...
                "source_decoded_pixel_sha256": frame["source_decoded_pixel_sha256"],
            }
        )
        ...
        with Image.open(temporary) as reopened:
            ...
            for placement, frame in zip(placements, frames, strict=True):
                ...
                if placement["pixel_identical"] is not True:
                    raise ReviewContactSheetError("review_contact_sheet_crop_pixel_identity_failed")
    except (OSError, SyntaxError) as exc:
    ...
    *, raw_result_path: str | Path, output_root: str | Path
    ...
    if (
        ...
        or (
            ...
            and raw.get("result_digest") != canonical_digest(raw, digest_field="result_digest")
        )
        or (
            raw.get("schema_version") == "public_scene_artifixer3d_final_composite.v1"
            and raw.get("receipt_digest") != canonical_digest(raw, digest_field="receipt_digest")
        )
        or (
            raw.get("schema_version") == "public_scene_agent_cad_replacement_visual_review.v1"
            and raw.get("receipt_digest") != canonical_digest(raw, digest_field="receipt_digest")
        )
        ...
        json.dumps(
            {
                ...
                "display_paths": [
                    path for task in result["tasks"] for path in task["display_paths"]
                ],
            },
            sort_keys=True,
        )
