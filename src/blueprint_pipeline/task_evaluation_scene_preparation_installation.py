...
def install_scene_preparation(*, bootstrap_path):
    ...
    config = {"schema_version": "task_evaluation_scene_progression_config.v1", "managed_by": MANAGED_BY,
        ...
        # Recovery and release succession reopen the same canonical queues,
        # ownership roots and fresh provider guard used by normal dispatch.
        "child_queue_root": str(state / "sam31-preparation-executions"),
        "child_execution_root": str(inputs / "sam31-preparations"),
        "launch_queue_root": str(state / "task-evaluation-launches"),
        "launch_execution_root": str(state / "task-evaluation-launch-runs"),
        "provider_guard_path": str(state / "gpu_spend_guard/latest.json"),
        "ownership_roots": [str(inputs), str(state)],
