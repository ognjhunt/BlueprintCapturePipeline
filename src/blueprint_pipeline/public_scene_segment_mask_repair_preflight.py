...
def _task_inputs(
    *, candidate_root: Path, row: Mapping[str, Any]
    ...
    if (
        ...
        or sweep.get("freeze_digest")
        != canonical_digest(sweep, digest_field="freeze_digest")
    ...
    for camera_id in sorted(cameras):
        ...
        image_path = _bound_absolute(
            images[camera_id], code="segment_repair_source_frame_invalid"
        )
        try:
        ...
        if (
            ...
            raise SegmentMaskRepairPreflightError(
                ["segment_repair_frame_or_mask_invalid"]
            )
...
def materialize_segment_mask_repair_preflight(
    *,
    ...
    candidate_path = _file(
        segment_cutout_set_path, code="segment_repair_candidate_set_missing"
    )
    ...
    if publisher_scene_ids != {str(authority["publisher_scene_id"])}:
        raise SegmentMaskRepairPreflightError(
            ["segment_repair_execution_authority_scene_mismatch"]
        )
