"""Retain bounded, secret-clean evidence from the nested ArtiFixer runtime."""
...
SCHEMA_VERSION = "task_evaluation_scene_configuration_artifixer_failure_evidence.v1"
...
_MARKER_PREFIX = "BLUEPRINT_PUBLIC_SCENE_ARTIFIXER3D_"
_PROVIDER_EXCLUDED_DIRECTORY_NAMES = frozenset({"artifixer_execution", "artifixer_output"})
...
class ArtifixerRuntimeFailureEvidenceError(RuntimeError):
...
def _blockers(runtime_result: Mapping[str, Any] | None, *, secrets: Sequence[str]) -> list[str]:
...
def _runtime_result_copy(
    ...
    # Scrub each string independently so URL redaction cannot consume JSON
    # syntax. Setup failures have a small typed result; retaining the complete
    # structure prevents another caller/producer mismatch from hiding the field
...
def retain_artifixer_runtime_failure_evidence(
    *,
    ...
    """Seal the inner refusal where the provider ZIP will retain it.

    The nested runtime's large execution/output directories are deliberately
    excluded from the provider ZIP. ``destination`` therefore belongs directly
    under ``released_artifixer_runtime`` and must not live under either one.
    """
    ...
    if output.exists() or output.is_symlink():
        raise ValueError("scene_configuration_artifixer_failure_evidence_exists")
    if not _PROVIDER_EXCLUDED_DIRECTORY_NAMES.isdisjoint(output.parts):
        raise ValueError("scene_configuration_artifixer_failure_evidence_path_invalid")
    output.parent.mkdir(parents=True, exist_ok=True)
    source_present = (
        not unresolved_source.is_symlink() and source.is_file() and not source.is_symlink()
    )
    secrets = tuple(str(value) for value in secret_values if str(value))
    blockers = _blockers(runtime_result, secrets=secrets)
    result_copy, result_truncated = _runtime_result_copy(runtime_result, secrets=secrets)
    ...
    value: dict[str, Any] = {
        ...
        "runtime_result_source_size_bytes": (source.stat().st_size if source_present else None),
        "runtime_result_source_sha256": (
            "sha256:" + hashlib.sha256(source.read_bytes()).hexdigest() if source_present else None
        ),
...
def read_artifixer_runtime_result(
    *,
    completed: Any,
    runtime_result_path: str | Path,
    ...
    try:
        ...
        raise ArtifixerRuntimeFailureEvidenceError(
            "scene_configuration_artifixer_runtime_result_missing"
        ) from exc
    ...
    evidence = retain_artifixer_runtime_failure_evidence(
        destination=evidence_path,
        completed=completed,
        runtime_result_path=path,
    ...
    raise ArtifixerRuntimeFailureEvidenceError(
        "scene_configuration_artifixer_runtime_failed:" + str(evidence["failure_detail"])
    )
