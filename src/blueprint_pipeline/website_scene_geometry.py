...
def run_website_scene_geometry(*, source_video: Path, output_root: Path, capture_id: str) -> dict[str, Any]:
    """Retain unedited observations and infer geometry once from local weights.

    No provider upload, model download, GPU rental, or synthetic fallback occurs
    here. Deployment installs the Apache checkpoint in the existing worker.
    """
    ...
    binding = {
        "source_video_digest": _sha256_file(source_video), "model_id": MODEL_ID,
        "model_code_revision": MODEL_CODE_REVISION, "adapter_version": 1,
        "checkpoint_digest": _sha256_file(weights), "model_config_digest": _sha256_file(config),
    }
    output_root = output_root.resolve()
    manifest_path = output_root / "source_geometry.json"
    if manifest_path.is_file():
        previous = json.loads(manifest_path.read_text())
        if previous.get("binding") == binding and previous.get("status") == "estimated":
            if previous.get("digest") != canonical_digest(previous, digest_field="digest"):
                raise ValueError("website_source_geometry_digest_mismatch")
            for frame in previous["frames"]:
                for path_key, digest_key in (("source_image_path", "source_image_digest"),
    ...
    for index, row in enumerate(candidate["frames"]):
        ...
        prepared.append({
            ...
            "source_image_path": str(source), "source_image_digest": row["frame_digest"],
            **_prepare_image(source, output_root / "model_inputs" / f"{index:06d}.png", rotation),
        })
    ...
    result = {
        ...
        "frames": frames, "manifest_path": str(manifest_path),
    }
