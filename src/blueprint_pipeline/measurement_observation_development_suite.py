"""Execute the checked Capture-to-Observation development corpus.

This orchestration helper turns the synthetic public corpus into exact
benchmark cases and runs each case through the uniform local adapter boundary.
The resulting suite is useful regression evidence, but its contract makes it
impossible to represent the run as held-out, physical, R5, R6, or R7 evidence.
"""
...
from .measurement_opencv_observation_adapter import (
    IMPLEMENTATION_ID,
...
SUITE_SCHEMA_VERSION = "capture_to_observation_development_suite.v1"
CORPUS_SCHEMA_VERSION = "capture_to_observation_development_corpus.v1"
EXPECTED_OPENCV_RUNTIME_VERSION = "4.11.0"


class ObservationDevelopmentSuiteError(ValueError):
...
def _load_corpus(path: Path) -> dict[str, Any]:
    try:
        ...
        raise ObservationDevelopmentSuiteError("observation_development_corpus_unreadable") from exc
    if not isinstance(value, Mapping):
        raise ObservationDevelopmentSuiteError("observation_development_corpus_not_object")
    corpus = dict(value)
    errors: list[str] = []
    if corpus.get("schema_version") != CORPUS_SCHEMA_VERSION:
        errors.append("observation_development_corpus_schema_invalid")
    for key, expected in (
        ...
        if corpus.get(key) is not expected:
            errors.append(f"observation_development_corpus_{key}_invalid")
    shared = corpus.get("shared_operating_point")
    cases = corpus.get("cases")
    if not isinstance(shared, Mapping):
        errors.append("observation_development_corpus_shared_point_invalid")
    if (
        not isinstance(cases, list)
        or len(cases) < 2
        or not all(isinstance(row, Mapping) for row in cases)
    ):
        errors.append("observation_development_corpus_cases_invalid")
    elif len({str(row.get("case_id", "")).strip() for row in cases}) != len(cases):
        errors.append("observation_development_corpus_case_ids_duplicate")
    if errors:
        raise ObservationDevelopmentSuiteError(*errors)
    return corpus


def _number(value: Any, code: str) -> float:
    if isinstance(value, bool):
        raise ObservationDevelopmentSuiteError(code)
    try:
        result = float(value)
    except (TypeError, ValueError) as exc:
        raise ObservationDevelopmentSuiteError(code) from exc
    if not math.isfinite(result):
        raise ObservationDevelopmentSuiteError(code)
    return result


def run_capture_to_observation_development_suite(
    corpus_path: Path,
    *,
    ...
    """Plan or run every public development case through the OpenCV worker."""

    if not _valid_digest(qualification_split_digest):
        raise ObservationDevelopmentSuiteError(
            "observation_development_qualification_split_digest_invalid"
        )
    if not _valid_digest(controller_scope_digest):
        raise ObservationDevelopmentSuiteError(
            "observation_development_controller_scope_digest_invalid"
        )
    ...
    if qualification_split_digest == corpus_digest:
        raise ObservationDevelopmentSuiteError("observation_development_split_leakage")
    ...
    for index, raw_case in enumerate(corpus["cases"]):
        ...
        request = build_measurement_adapter_execution_request(
            ...
            spec,
            case,
            execution_id=f"opencv-observation-{index + 1:03d}-{case_id}",
            ...
            seed=17,
            solver_settings={
                "opencv_version": EXPECTED_OPENCV_RUNTIME_VERSION,
        ...
        summaries.append(
            {
                ...
                "deterministic_replay_match": bundle["receipt"]
    ...
    metric_rows = [row["observed_metrics"] for row in summaries]
    aggregate_metrics: dict[str, float] = {}
    if completed:
        reprojection = [
            _number(
                row["calibrated_image_depth_lidar_residuals"],
                "observation_development_reprojection_metric_invalid",
            )
            for row in metric_rows
        ]
        missing_depth = [
            _number(
                row["missing_depth_distribution"],
                "observation_development_missing_depth_metric_invalid",
            )
            for row in metric_rows
        ]
        temporal = [
            _number(
                row["temporal_error"],
                "observation_development_temporal_metric_invalid",
            )
            for row in metric_rows
        ]
        aggregate_metrics = {
            "mean_reprojection_rmse_px": fmean(reprojection),
            "maximum_reprojection_rmse_px": max(reprojection),
            "mean_missing_depth_fraction": fmean(missing_depth),
            "maximum_temporal_error_ms": max(temporal),
        }
    suite = {
    ...
    return validate_capture_to_observation_development_suite(suite)


def validate_capture_to_observation_development_suite(
    value: Mapping[str, Any],
) -> dict[str, Any]:
    suite = json.loads(json.dumps(dict(value)))
    errors: list[str] = []
    if suite.get("schema_version") != SUITE_SCHEMA_VERSION:
        errors.append("observation_development_suite_schema_invalid")
    if suite.get("status") not in {
        "planned_not_executed",
        "completed_development_only",
        "incomplete",
    }:
        errors.append("observation_development_suite_status_invalid")
    cases = suite.get("cases")
    if not isinstance(cases, list) or len(cases) < 2:
        errors.append("observation_development_suite_cases_invalid")
    if suite.get("case_count") != len(cases or []):
        errors.append("observation_development_suite_case_count_mismatch")
    for key, expected in (
        ("development_only", True),
        ("synthetic_fixture", True),
        ("held_out", False),
        ...
        if suite.get(key) is not expected:
            errors.append(f"observation_development_suite_{key}_invalid")
    if suite.get("status") == "completed_development_only":
        if suite.get("all_cases_completed") is not True:
            errors.append("observation_development_suite_completion_invalid")
        if suite.get("all_replays_deterministic") is not True:
            errors.append("observation_development_suite_replay_invalid")
        if (
            not isinstance(suite.get("aggregate_metrics"), Mapping)
            or not suite["aggregate_metrics"]
        ):
            errors.append("observation_development_suite_metrics_missing")
    ...
    if suite.get("suite_digest") is not None and suite.get("suite_digest") != expected_digest:
        errors.append("observation_development_suite_digest_mismatch")
    if errors:
        raise ObservationDevelopmentSuiteError(*errors)
...
def main(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Plan or run the Capture-to-Observation development corpus"
    )
    ...
    suite = run_capture_to_observation_development_suite(
        args.corpus,
...
__all__ = [
    ...
    "EXPECTED_OPENCV_RUNTIME_VERSION",
