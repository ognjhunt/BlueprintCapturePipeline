...
def buyer_readout_projection(closure: Mapping[str, Any]) -> dict[str, Any]:
    ...
    value = _mapping(closure)
    valid = value.get("schema_version") == SCHEMA_VERSION and value.get("status") == "completed"
    rows = {
    ...
    return {
        ...
        "task_success_proven": bool(valid and rows.get("persistent_simulator_transition") == "passed"),
        "semantic_review_passed": bool(valid and rows.get("semantic_review") == "passed"),
        "forward_consistency_passed": bool(valid and rows.get("forward_consistency") == "passed"),
        "inverse_consistency_passed": bool(valid and rows.get("inverse_consistency") == "passed"),
        "teardown_and_zero_inventory_proven": bool(
            valid and rows.get("teardown") == "passed" and rows.get("final_inventory") == "passed"
        ),
        "blockers": [] if valid else ["g1_kitchen_attempt_closure_not_completed"],
    }
