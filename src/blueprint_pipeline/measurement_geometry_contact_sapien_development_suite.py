"""Plan or execute the SAPIEN PhysX rigid-contact development corpus."""
...
from .measurement_sapien_rigid_adapter import (
    IMPLEMENTATION_ID,
...
SUITE_SCHEMA_VERSION = "capture_to_geometry_contact_sapien_development_suite.v1"
METHOD_ID = "sapien-maniskill-3"
SOLVER_SCOPE = "sapien-physx-cpu-tgs-enhanced-determinism"


class GeometryContactSapienDevelopmentSuiteError(GeometryContactDevelopmentSuiteError):
    """Raised when the SAPIEN corpus suite fails closed validation."""


def _suite_error(*codes: str) -> GeometryContactSapienDevelopmentSuiteError:
    return GeometryContactSapienDevelopmentSuiteError(*codes)


def run_capture_to_geometry_contact_sapien_development_suite(
    corpus_path: Path,
    *,
    ...
    """Run every public rigid-contact case through SAPIEN's local CPU port."""

    if not _valid_digest(qualification_split_digest):
        raise _suite_error("geometry_contact_sapien_qualification_split_digest_invalid")
    if not _valid_digest(controller_scope_digest):
        raise _suite_error("geometry_contact_sapien_controller_scope_digest_invalid")
    ...
    if qualification_split_digest == corpus_digest:
        raise _suite_error("geometry_contact_sapien_split_leakage")
    spec = build_qualification_benchmark_spec(
        benchmark_id="capture-to-geometry-and-contact",
        benchmark_version="development-sapien-physx-rigid-contact-1",
    ...
    for index, raw_case in enumerate(corpus["cases"]):
        ...
        request = build_measurement_adapter_execution_request(
            ...
            spec,
            case,
            execution_id=f"sapien-rigid-contact-{index + 1:03d}-{case_id}",
        ...
        summaries.append(
            {
                ...
                "renderer_created": runtime.get("renderer_created"),
                "maniskill_runtime_used": runtime.get("maniskill_runtime_used"),
                "physx_version": runtime.get("physx_version"),
                "observed_metrics": (
    ...
    if completed:
        penetration = [
            _number(
                ...
                "geometry_contact_sapien_penetration_metric_invalid",
            )
        ...
        contact_times = [
            _number(
                ...
                "geometry_contact_sapien_contact_time_invalid",
            )
            for row in summaries
        ]
        aggregate_metrics = {
    ...
    return validate_capture_to_geometry_contact_sapien_development_suite(suite)


def validate_capture_to_geometry_contact_sapien_development_suite(
    value: Mapping[str, Any],
) -> dict[str, Any]:
    suite = json.loads(json.dumps(dict(value)))
    errors: list[str] = []
    if suite.get("schema_version") != SUITE_SCHEMA_VERSION:
        errors.append("geometry_contact_sapien_suite_schema_invalid")
    if suite.get("corpus_schema_version") != CORPUS_SCHEMA_VERSION:
        errors.append("geometry_contact_sapien_corpus_schema_invalid")
    for key, expected in (
        ("lane", "rigid_contact"),
        ("method_id", METHOD_ID),
        ("solver_scope", SOLVER_SCOPE),
        ...
        if suite.get(key) != expected:
            errors.append(f"geometry_contact_sapien_suite_{key}_invalid")
    if suite.get("status") not in {
        "planned_not_executed",
        "completed_development_only",
        "incomplete",
    }:
        errors.append("geometry_contact_sapien_suite_status_invalid")
    cases = suite.get("cases")
    if not isinstance(cases, list) or len(cases) < 2:
        errors.append("geometry_contact_sapien_suite_cases_invalid")
    if suite.get("case_count") != len(cases or []):
        errors.append("geometry_contact_sapien_suite_case_count_mismatch")
    if isinstance(cases, list):
        if any(row.get("renderer_created") is True for row in cases):
            errors.append("geometry_contact_sapien_suite_renderer_case_invalid")
        if any(row.get("maniskill_runtime_used") is True for row in cases):
            errors.append("geometry_contact_sapien_suite_maniskill_case_invalid")
    if suite.get("status") == "completed_development_only":
        if suite.get("all_cases_completed") is not True:
            errors.append("geometry_contact_sapien_suite_completion_invalid")
        if suite.get("all_replays_deterministic") is not True:
            errors.append("geometry_contact_sapien_suite_replay_invalid")
        if (
            not isinstance(suite.get("aggregate_metrics"), Mapping)
            or not suite["aggregate_metrics"]
        ):
            errors.append("geometry_contact_sapien_suite_metrics_missing")
    ...
    if suite.get("suite_digest") is not None and suite.get("suite_digest") != expected_digest:
        errors.append("geometry_contact_sapien_suite_digest_mismatch")
    if errors:
...
def main(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Plan or run the SAPIEN Capture-to-Geometry-and-Contact corpus"
    )
    ...
    suite = run_capture_to_geometry_contact_sapien_development_suite(
        args.corpus,
