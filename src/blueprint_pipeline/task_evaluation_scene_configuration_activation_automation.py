...
def provision_scene_configuration_activation_intent(
    *,
    ...
    # Every deploy changes the commit; each release keeps its own immutable inputs.
    inputs = Path(materialization_root).expanduser() / identity / expected_production_commit
    template = {
