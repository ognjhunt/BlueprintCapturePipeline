...
def _blueprint_bundle_preflight(
    *,
    ...
    adp_simpler_required_entries = {
        "provider_runtime/run_adp_simpler_provider_runtime.sh",
        "provider_runtime/adp_simpler_provider_runner.py",
    ...
    adp_aura_smoke_required_entries = {
        ...
        "provider_runtime/adp_aura_author_smoke_provider_manifest.json",
        "provider_runtime/sam2_source.zip",
        "provider_runtime/aurafusion360_source.zip",
        "provider_runtime/smoke_spec.json",
    }
    adp_aura_interiorgs_required_entries = {
        "provider_runtime/run_adp_aura_interiorgs_provider_runtime.sh",
        "provider_runtime/adp_aura_interiorgs_provider_runner.py",
        "provider_runtime/adp_aura_interiorgs_provider_manifest.json",
        "provider_runtime/sam2_source.zip",
        "provider_runtime/aurafusion360_source.zip",
        "provider_runtime/lama_source.zip",
        "provider_runtime/big-lama.zip",
        "provider_runtime/interiorgs_adapter.zip",
        "provider_runtime/execution_spec.json",
    }
    adp_aura_exact_residual_required_entries = {
        "provider_runtime/run_public_scene_aura_exact_residual.sh",
        "provider_runtime/public_scene_aura_exact_residual_runner.py",
        "provider_runtime/aura_exact_residual_bundle_manifest.json",
        "provider_runtime/aura_exact_residual_runtime_request.json",
        "provider_runtime/input/preflight.json",
        "provider_runtime/input/backend_admission.json",
    ...
    if provider_bundle_kind in {"isaac", "adp_simready_isaac"}:
        ...
        required_entries = adp_simpler_required_entries
        ...
        required_entries = adp_aura_interiorgs_required_entries
        entrypoint_member = "provider_runtime/run_adp_aura_interiorgs_provider_runtime.sh"
        runner_member = "provider_runtime/adp_aura_interiorgs_provider_runner.py"
        readiness_name = "adp_aura_interiorgs_provider_manifest.json"
    elif provider_bundle_kind == "adp_aura_exact_residual":
        required_entries = adp_aura_exact_residual_required_entries
                    ...
                    if provider_bundle_kind == "adp_gaussian_excision":
                        ...
                        try:
                            dependency_manifest = json.loads(
                                archive.read("dependency_wheelhouse_manifest.json").decode("utf-8")
                            )
                            ...
                            if (
                                ...
                                or dependency_manifest.get("provider_network_install_required")
                                is not False
                                or not isinstance(dependency_rows, list)
                                or not dependency_rows
                            ):
                                raise ValueError("gaussian_excision_dependency_manifest_invalid")
                            for dependency_row in dependency_rows:
                                ...
                                filename = str(dependency_row.get("filename") or "")
...
def _probe_shell_script(
    ...
    *,
    ...
    elif enable_blueprint_bundle and provider_bundle_kind == "adp_retained_scene_render":
        # The pinned Playwright image carries its browser/runtime closure.  The
        # bundle's Node runner independently rejects software WebGL after the
        # preflight nvidia-smi check; apt would violate its zero-install term.
        script += "cuda_runtime_rc=0; echo BLUEPRINT_VAST_CUDA_RUNTIME_OK:node_webgl_runtime; "
    else:
    ...
    if enable_blueprint_bundle:
        ...
        if provider_bundle_kind in {"isaac", "adp_simready_isaac"}:
            ...
            script += (
                ...
                "else "
                'rm -rf "$WORK_DIR/adp_simpler_provider_bundle" "$WORK_DIR/adp_simpler_provider_runtime_bundle.zip" "$WORK_DIR/adp_simpler_provider_runtime_output.zip"; '
                'blueprint_download_url "$BUNDLE_URL" "$WORK_DIR/adp_simpler_provider_runtime_bundle.zip"; dl=$?; '
                "if [ $dl -ne 0 ]; then echo BLUEPRINT_VAST_PROVIDER_BUNDLE_BLOCKED:download_failed:$dl; "
                "else echo BLUEPRINT_VAST_PROVIDER_BUNDLE_DOWNLOADED; "
                '$RUNTIME_PY -m zipfile -e "$WORK_DIR/adp_simpler_provider_runtime_bundle.zip" "$WORK_DIR/adp_simpler_provider_bundle"; unzip_rc=$?; '
                "if [ $unzip_rc -ne 0 ]; then echo BLUEPRINT_VAST_PROVIDER_BUNDLE_BLOCKED:unzip_failed:$unzip_rc; "
                'elif [ ! -f "$WORK_DIR/adp_simpler_provider_bundle/provider_runtime/run_adp_simpler_provider_runtime.sh" ]; then echo BLUEPRINT_VAST_PROVIDER_BUNDLE_BLOCKED:entrypoint_missing; '
                "else "
                'export BLUEPRINT_ADP_SIMPLER_OUTPUT_DIR="$WORK_DIR/adp_simpler_provider_bundle/runtime_output"; '
                'mkdir -p "$BLUEPRINT_ADP_SIMPLER_OUTPUT_DIR"; '
                "echo BLUEPRINT_VAST_PROVIDER_ENTRYPOINT_STARTED; "
                'bash "$WORK_DIR/adp_simpler_provider_bundle/provider_runtime/run_adp_simpler_provider_runtime.sh"; provider_rc=$?; '
                "echo BLUEPRINT_VAST_PROVIDER_ENTRYPOINT_EXIT_CODE:$provider_rc; "
                "$RUNTIME_PY - <<'PY'\n"
                "import json\n"
                "import os\n"
                "import zipfile\n"
                "from pathlib import Path\n"
                "output_dir = Path(os.environ.get('BLUEPRINT_ADP_SIMPLER_OUTPUT_DIR', '/workspace/adp_simpler_provider_bundle/runtime_output'))\n"
                "work_dir = Path(os.environ.get('BLUEPRINT_VAST_WORK_DIR', '/tmp/blueprint_vast_work'))\n"
                "output_zip = work_dir / 'adp_simpler_provider_runtime_output.zip'\n"
                "with zipfile.ZipFile(output_zip, 'w', compression=zipfile.ZIP_DEFLATED) as archive:\n"
                "    if output_dir.is_dir():\n"
                "        for path in sorted(output_dir.rglob('*')):\n"
                "            if path.is_file():\n"
            ...
            script += (
                common_start + "RUNTIME_PY=''; "
                "if command -v apt-get >/dev/null 2>&1; then "
                "apt-get update >/tmp/blueprint_adp_aura_apt_update.log 2>&1 && "
                "DEBIAN_FRONTEND=noninteractive apt-get install -y python3 python3-venv python3-pip curl unzip git build-essential gcc-11 g++-11 ffmpeg libgl1 libglib2.0-0 >/tmp/blueprint_adp_aura_apt_install.log 2>&1; "
                "fi; "
                "if [ -x /usr/bin/python3 ]; then RUNTIME_PY=/usr/bin/python3; "
                "elif command -v python3 >/dev/null 2>&1; then RUNTIME_PY=$(command -v python3); fi; "
                'if [ -z "$RUNTIME_PY" ]; then echo BLUEPRINT_VAST_PROVIDER_BUNDLE_BLOCKED:python_missing; '
                "else "
                'rm -rf "$WORK_DIR/adp_aura_provider_bundle" "$WORK_DIR/adp_aura_provider_runtime_bundle.zip" "$WORK_DIR/adp_aura_provider_runtime_output.zip"; '
                'blueprint_download_url "$BUNDLE_URL" "$WORK_DIR/adp_aura_provider_runtime_bundle.zip"; dl=$?; '
                "if [ $dl -ne 0 ]; then echo BLUEPRINT_VAST_PROVIDER_BUNDLE_BLOCKED:download_failed:$dl; "
                "else echo BLUEPRINT_VAST_PROVIDER_BUNDLE_DOWNLOADED; "
                '$RUNTIME_PY -m zipfile -e "$WORK_DIR/adp_aura_provider_runtime_bundle.zip" "$WORK_DIR/adp_aura_provider_bundle"; unzip_rc=$?; '
                "if [ $unzip_rc -ne 0 ]; then echo BLUEPRINT_VAST_PROVIDER_BUNDLE_BLOCKED:unzip_failed:$unzip_rc; "
                'elif [ ! -f "$WORK_DIR/adp_aura_provider_bundle/provider_runtime/run_adp_aura_author_smoke_provider_runtime.sh" ]; then echo BLUEPRINT_VAST_PROVIDER_BUNDLE_BLOCKED:entrypoint_missing; '
                "else "
                'export BLUEPRINT_ADP_AURA_OUTPUT_DIR="$WORK_DIR/adp_aura_provider_bundle/runtime_output"; '
                'mkdir -p "$BLUEPRINT_ADP_AURA_OUTPUT_DIR"; '
                "echo BLUEPRINT_VAST_PROVIDER_ENTRYPOINT_STARTED; "
                'bash "$WORK_DIR/adp_aura_provider_bundle/provider_runtime/run_adp_aura_author_smoke_provider_runtime.sh"; provider_rc=$?; '
                "echo BLUEPRINT_VAST_PROVIDER_ENTRYPOINT_EXIT_CODE:$provider_rc; "
                "$RUNTIME_PY - <<'PY'\n"
                "import json\n"
                "import os\n"
                "import zipfile\n"
                "from pathlib import Path\n"
                "output_dir = Path(os.environ.get('BLUEPRINT_ADP_AURA_OUTPUT_DIR', '/workspace/adp_aura_provider_bundle/runtime_output'))\n"
                "work_dir = Path(os.environ.get('BLUEPRINT_VAST_WORK_DIR', '/tmp/blueprint_vast_work'))\n"
                "output_zip = work_dir / 'adp_aura_provider_runtime_output.zip'\n"
                "with zipfile.ZipFile(output_zip, 'w', compression=zipfile.ZIP_DEFLATED) as archive:\n"
                "    if output_dir.is_dir():\n"
                "        for path in sorted(output_dir.rglob('*')):\n"
                "            if path.is_file() and path.stat().st_size <= 300_000_000:\n"
            ...
            script += (
                common_start + "RUNTIME_PY=''; "
                "if command -v apt-get >/dev/null 2>&1; then "
                "apt-get update >/tmp/blueprint_adp_aura_interiorgs_apt_update.log 2>&1 && "
                "DEBIAN_FRONTEND=noninteractive apt-get install -y python3 python3-venv python3-pip curl unzip git build-essential gcc-11 g++-11 ffmpeg libgl1 libglib2.0-0 >/tmp/blueprint_adp_aura_interiorgs_apt_install.log 2>&1; "
                "fi; "
                "if [ -x /usr/bin/python3 ]; then RUNTIME_PY=/usr/bin/python3; "
                "elif command -v python3 >/dev/null 2>&1; then RUNTIME_PY=$(command -v python3); fi; "
                'if [ -z "$RUNTIME_PY" ]; then echo BLUEPRINT_VAST_PROVIDER_BUNDLE_BLOCKED:python_missing; '
                "else "
                'rm -rf "$WORK_DIR/adp_aura_interiorgs_provider_bundle" "$WORK_DIR/adp_aura_interiorgs_provider_runtime_bundle.zip" "$WORK_DIR/adp_aura_interiorgs_provider_runtime_output.zip"; '
                'blueprint_download_url "$BUNDLE_URL" "$WORK_DIR/adp_aura_interiorgs_provider_runtime_bundle.zip"; dl=$?; '
                "if [ $dl -ne 0 ]; then echo BLUEPRINT_VAST_PROVIDER_BUNDLE_BLOCKED:download_failed:$dl; "
                "else echo BLUEPRINT_VAST_PROVIDER_BUNDLE_DOWNLOADED; "
                '$RUNTIME_PY -m zipfile -e "$WORK_DIR/adp_aura_interiorgs_provider_runtime_bundle.zip" "$WORK_DIR/adp_aura_interiorgs_provider_bundle"; unzip_rc=$?; '
                "if [ $unzip_rc -ne 0 ]; then echo BLUEPRINT_VAST_PROVIDER_BUNDLE_BLOCKED:unzip_failed:$unzip_rc; "
                'elif [ ! -f "$WORK_DIR/adp_aura_interiorgs_provider_bundle/provider_runtime/run_adp_aura_interiorgs_provider_runtime.sh" ]; then echo BLUEPRINT_VAST_PROVIDER_BUNDLE_BLOCKED:entrypoint_missing; '
                "else "
                'export BLUEPRINT_ADP_AURA_INTERIORGS_OUTPUT_DIR="$WORK_DIR/adp_aura_interiorgs_provider_bundle/runtime_output"; '
                'mkdir -p "$BLUEPRINT_ADP_AURA_INTERIORGS_OUTPUT_DIR"; '
                "echo BLUEPRINT_VAST_PROVIDER_ENTRYPOINT_STARTED; "
                'bash "$WORK_DIR/adp_aura_interiorgs_provider_bundle/provider_runtime/run_adp_aura_interiorgs_provider_runtime.sh"; provider_rc=$?; '
                "echo BLUEPRINT_VAST_PROVIDER_ENTRYPOINT_EXIT_CODE:$provider_rc; "
                "$RUNTIME_PY - <<'PY'\n"
                "import json\n"
                "import os\n"
                "import zipfile\n"
                "from pathlib import Path\n"
                "output_dir = Path(os.environ.get('BLUEPRINT_ADP_AURA_INTERIORGS_OUTPUT_DIR', '/workspace/adp_aura_interiorgs_provider_bundle/runtime_output'))\n"
                "work_dir = Path(os.environ.get('BLUEPRINT_VAST_WORK_DIR', '/tmp/blueprint_vast_work'))\n"
                "output_zip = work_dir / 'adp_aura_interiorgs_provider_runtime_output.zip'\n"
                "with zipfile.ZipFile(output_zip, 'w', compression=zipfile.ZIP_DEFLATED) as archive:\n"
                "    if output_dir.is_dir():\n"
                "        skipped = []\n"
                "        for path in sorted(output_dir.rglob('*')):\n"
                ...
                "        if skipped:\n"
                "            archive.writestr('provider_output_zip_exclusions.json', json.dumps({'schema_version': 'adp_aura_interiorgs_provider_output_zip_exclusions.v1', 'skipped': skipped}, indent=2, sort_keys=True))\n"
                "    else:\n"
                "        archive.writestr('runtime_output_missing.json', json.dumps({'status': 'blocked', 'blockers': ['runtime_output_directory_missing']}, indent=2))\n"
                "print('BLUEPRINT_VAST_PROVIDER_OUTPUT_ZIP_WRITTEN:%d' % output_zip.stat().st_size)\n"
                "PY\n"
            ...
            # The exact-residual packet is intentionally a separate provider
            # protocol.  It accepts only the sealed private-derived ZIP and
            # returns only its deterministic runtime output; it never invokes
            # the Inpaint360 or historical InteriorGS entrypoints.
            script += (
                common_start + "RUNTIME_PY=''; "
                "if command -v apt-get >/dev/null 2>&1; then "
                "apt-get update >/tmp/blueprint_adp_aura_exact_residual_apt_update.log 2>&1 && "
                "DEBIAN_FRONTEND=noninteractive apt-get install -y python3 python3-venv python3-pip curl unzip git build-essential gcc-11 g++-11 ffmpeg libgl1 libglib2.0-0 >/tmp/blueprint_adp_aura_exact_residual_apt_install.log 2>&1; "
                "fi; "
                "if [ -x /usr/bin/python3 ]; then RUNTIME_PY=/usr/bin/python3; "
                "elif command -v python3 >/dev/null 2>&1; then RUNTIME_PY=$(command -v python3); fi; "
                'if [ -z "$RUNTIME_PY" ]; then echo BLUEPRINT_VAST_PROVIDER_BUNDLE_BLOCKED:python_missing; '
                "else "
                'rm -rf "$WORK_DIR/adp_aura_exact_residual_provider_bundle" "$WORK_DIR/adp_aura_exact_residual_provider_runtime_bundle.zip" "$WORK_DIR/adp_aura_exact_residual_provider_runtime_output.zip"; '
                'blueprint_download_url "$BUNDLE_URL" "$WORK_DIR/adp_aura_exact_residual_provider_runtime_bundle.zip"; dl=$?; '
                "if [ $dl -ne 0 ]; then echo BLUEPRINT_VAST_PROVIDER_BUNDLE_BLOCKED:download_failed:$dl; "
                "else echo BLUEPRINT_VAST_PROVIDER_BUNDLE_DOWNLOADED; "
                '$RUNTIME_PY -m zipfile -e "$WORK_DIR/adp_aura_exact_residual_provider_runtime_bundle.zip" "$WORK_DIR/adp_aura_exact_residual_provider_bundle"; unzip_rc=$?; '
                "if [ $unzip_rc -ne 0 ]; then echo BLUEPRINT_VAST_PROVIDER_BUNDLE_BLOCKED:unzip_failed:$unzip_rc; "
                'elif [ ! -f "$WORK_DIR/adp_aura_exact_residual_provider_bundle/provider_runtime/run_public_scene_aura_exact_residual.sh" ]; then echo BLUEPRINT_VAST_PROVIDER_BUNDLE_BLOCKED:entrypoint_missing; '
                "else "
                'export BLUEPRINT_PUBLIC_SCENE_AURA_EXACT_RESIDUAL_OUTPUT_DIR="$WORK_DIR/adp_aura_exact_residual_provider_bundle/runtime_output"; '
                'mkdir -p "$BLUEPRINT_PUBLIC_SCENE_AURA_EXACT_RESIDUAL_OUTPUT_DIR"; '
                "echo BLUEPRINT_VAST_PROVIDER_ENTRYPOINT_STARTED; "
                'bash "$WORK_DIR/adp_aura_exact_residual_provider_bundle/provider_runtime/run_public_scene_aura_exact_residual.sh"; provider_rc=$?; '
                "echo BLUEPRINT_VAST_PROVIDER_ENTRYPOINT_EXIT_CODE:$provider_rc; "
                "$RUNTIME_PY - <<'PY'\n"
                "import json\n"
                "import os\n"
                "import zipfile\n"
                "from pathlib import Path\n"
                "output_dir = Path(os.environ.get('BLUEPRINT_PUBLIC_SCENE_AURA_EXACT_RESIDUAL_OUTPUT_DIR', '/workspace/adp_aura_exact_residual_provider_bundle/runtime_output'))\n"
                "work_dir = Path(os.environ.get('BLUEPRINT_VAST_WORK_DIR', '/tmp/blueprint_vast_work'))\n"
                "output_zip = work_dir / 'adp_aura_exact_residual_provider_runtime_output.zip'\n"
                "with zipfile.ZipFile(output_zip, 'w', compression=zipfile.ZIP_DEFLATED) as archive:\n"
                "    if output_dir.is_dir():\n"
                "        skipped = []\n"
                "        for path in sorted(output_dir.rglob('*')):\n"
                ...
                "                    skipped.append({'path': relative, 'size_bytes': size, 'reason': 'file_exceeds_provider_output_limit'})\n"
                "        if skipped:\n"
                "            archive.writestr('provider_output_zip_exclusions.json', json.dumps({'schema_version': 'adp_aura_exact_residual_provider_output_zip_exclusions.v1', 'skipped': skipped}, indent=2, sort_keys=True))\n"
                "    else:\n"
                "        archive.writestr('runtime_output_missing.json', json.dumps({'status': 'blocked', 'blockers': ['runtime_output_directory_missing']}, indent=2))\n"
                "print('BLUEPRINT_VAST_PROVIDER_OUTPUT_ZIP_WRITTEN:%d' % output_zip.stat().st_size)\n"
                "PY\n"
            ...
            script += (
                common_start + "RUNTIME_PY=''; "
                "if command -v python3 >/dev/null 2>&1; then RUNTIME_PY=$(command -v python3); "
                "elif command -v python >/dev/null 2>&1; then RUNTIME_PY=$(command -v python); fi; "
                'if [ -z "$RUNTIME_PY" ]; then echo BLUEPRINT_VAST_PROVIDER_BUNDLE_BLOCKED:python_missing; '
                "else "
                'rm -rf "$WORK_DIR/adp_retained_scene_render_provider_bundle" "$WORK_DIR/adp_retained_scene_render_provider_runtime_bundle.zip" "$WORK_DIR/adp_retained_scene_render_provider_runtime_output.zip"; '
                'blueprint_download_url "$BUNDLE_URL" "$WORK_DIR/adp_retained_scene_render_provider_runtime_bundle.zip"; dl=$?; '
                "if [ $dl -ne 0 ]; then echo BLUEPRINT_VAST_PROVIDER_BUNDLE_BLOCKED:download_failed:$dl; "
                "else echo BLUEPRINT_VAST_PROVIDER_BUNDLE_DOWNLOADED; "
                '$RUNTIME_PY -m zipfile -e "$WORK_DIR/adp_retained_scene_render_provider_runtime_bundle.zip" "$WORK_DIR/adp_retained_scene_render_provider_bundle"; unzip_rc=$?; '
                "if [ $unzip_rc -ne 0 ]; then echo BLUEPRINT_VAST_PROVIDER_BUNDLE_BLOCKED:unzip_failed:$unzip_rc; "
                'elif [ ! -f "$WORK_DIR/adp_retained_scene_render_provider_bundle/provider_runtime/run_adp_retained_scene_render_provider_runtime.sh" ]; then echo BLUEPRINT_VAST_PROVIDER_BUNDLE_BLOCKED:entrypoint_missing; '
                "else "
                'export BLUEPRINT_ADP_RETAINED_SCENE_RENDER_OUTPUT_DIR="$WORK_DIR/adp_retained_scene_render_provider_bundle/runtime_output"; '
                'mkdir -p "$BLUEPRINT_ADP_RETAINED_SCENE_RENDER_OUTPUT_DIR"; '
                "echo BLUEPRINT_VAST_PROVIDER_ENTRYPOINT_STARTED; "
                'bash "$WORK_DIR/adp_retained_scene_render_provider_bundle/provider_runtime/run_adp_retained_scene_render_provider_runtime.sh"; provider_rc=$?; '
                "echo BLUEPRINT_VAST_PROVIDER_ENTRYPOINT_EXIT_CODE:$provider_rc; "
                "$RUNTIME_PY - <<'PY'\n"
                "import json\n"
                "import os\n"
                "import zipfile\n"
                "from pathlib import Path\n"
                "output_dir = Path(os.environ.get('BLUEPRINT_ADP_RETAINED_SCENE_RENDER_OUTPUT_DIR', '/workspace/adp_retained_scene_render_provider_bundle/runtime_output'))\n"
                "work_dir = Path(os.environ.get('BLUEPRINT_VAST_WORK_DIR', '/tmp/blueprint_vast_work'))\n"
                "output_zip = work_dir / 'adp_retained_scene_render_provider_runtime_output.zip'\n"
                "with zipfile.ZipFile(output_zip, 'w', compression=zipfile.ZIP_DEFLATED) as archive:\n"
                "    if output_dir.is_dir():\n"
                "        for path in sorted(output_dir.rglob('*')):\n"
                "            if path.is_file():\n"
