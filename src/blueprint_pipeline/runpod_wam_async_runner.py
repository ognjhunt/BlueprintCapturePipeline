...
RUNPOD_WAM_TERMINAL_HOLD_SECONDS_ENV = "BLUEPRINT_RUNPOD_WAM_TERMINAL_HOLD_SECONDS"
...
mkdir -p "$BLUEPRINT_WAM_PROVIDER_OUTPUT_DIR"
# Legacy provider bundles used the Vast-specific name.  Bind both names to the
# same provider-neutral directory so every runtime result and video is included
# in the terminal archive regardless of which provider launched it.
export BLUEPRINT_VAST_PROVIDER_OUTPUT_DIR="$BLUEPRINT_WAM_PROVIDER_OUTPUT_DIR"
...
terminal_hold_seconds="${BLUEPRINT_RUNPOD_WAM_TERMINAL_HOLD_SECONDS:-7200}"
echo "BLUEPRINT_RUNPOD_WAM_TERMINAL_HOLD_SECONDS=$terminal_hold_seconds"
# RunPod restarts a Pod whose main command exits.  Keep the terminal archive
# stable until the control-plane poller observes it and deletes/stops the Pod;
# the independent watchdog remains the hard backstop if the poller disappears.
sleep "$terminal_hold_seconds"
"""
