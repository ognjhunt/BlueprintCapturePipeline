"""Plan or execute the checked MuJoCo flex-cloth development corpus."""
...
from .measurement_mujoco_flex_cloth_adapter import (
    IMPLEMENTATION_ID,
...
SUITE_SCHEMA_VERSION = "capture_to_deformation_cloth_development_suite.v1"
CORPUS_SCHEMA_VERSION = "capture_to_deformation_cloth_development_corpus.v1"


class ClothDevelopmentSuiteError(ValueError):
...
def _number(value: Any, code: str) -> float:
    if isinstance(value, bool):
        raise ClothDevelopmentSuiteError(code)
    try:
        result = float(value)
    except (TypeError, ValueError) as exc:
        raise ClothDevelopmentSuiteError(code) from exc
...
def _load_corpus(path: Path) -> dict[str, Any]:
    try:
        ...
        raise ClothDevelopmentSuiteError("cloth_development_corpus_unreadable") from exc
    if not isinstance(value, Mapping):
        raise ClothDevelopmentSuiteError("cloth_development_corpus_not_object")
    corpus = dict(value)
    errors: list[str] = []
    if corpus.get("schema_version") != CORPUS_SCHEMA_VERSION:
        errors.append("cloth_development_corpus_schema_invalid")
    if corpus.get("lane") != "cloth":
        errors.append("cloth_development_corpus_lane_invalid")
    for key, expected in (
        ...
        if corpus.get(key) is not expected:
            errors.append(f"cloth_development_corpus_{key}_invalid")
    shared = corpus.get("shared_operating_point")
    cases = corpus.get("cases")
    if not isinstance(shared, Mapping):
        errors.append("cloth_development_corpus_shared_point_invalid")
    if (
        not isinstance(cases, list)
        or len(cases) < 2
        or not all(isinstance(row, Mapping) for row in cases)
    ):
        errors.append("cloth_development_corpus_cases_invalid")
    elif len({str(row.get("case_id", "")).strip() for row in cases}) != len(cases):
        errors.append("cloth_development_corpus_case_ids_duplicate")
    if errors:
        raise ClothDevelopmentSuiteError(*errors)
    return corpus


def run_capture_to_deformation_cloth_development_suite(
    corpus_path: Path,
    *,
    ...
    """Run all public cloth cases through the uniform local adapter boundary."""

    if not _valid_digest(qualification_split_digest):
        raise ClothDevelopmentSuiteError("cloth_development_qualification_split_digest_invalid")
    if not _valid_digest(controller_scope_digest):
        raise ClothDevelopmentSuiteError("cloth_development_controller_scope_digest_invalid")
    ...
    if qualification_split_digest == corpus_digest:
        raise ClothDevelopmentSuiteError("cloth_development_split_leakage")
    spec = build_qualification_benchmark_spec(
        benchmark_id="capture-to-deformation",
        benchmark_version="development-mujoco-flex-cloth-1",
        method_ids=["mujoco-3"],
    ...
    for index, raw_case in enumerate(corpus["cases"]):
        ...
        case = build_benchmark_case_manifest(
            ...
            task_class="cloth_folding_draping",
            material_regime="garment_cloth",
        ...
        request = build_measurement_adapter_execution_request(
            ...
            spec,
            case,
            execution_id=f"mujoco-flex-cloth-{index + 1:03d}-{case_id}",
            implementation_id=IMPLEMENTATION_ID,
            implementation_version=IMPLEMENTATION_VERSION,
            implementation_digest=implementation_digest(),
            backend_id="mujoco-cpu-flex-elastic2d-stretch",
            precision="float64",
            seed=29,
            solver_settings={
                "elastic2d": "stretch",
        ...
        bundle = run_measurement_adapter_execution(
            ...
            command_argv=[
                sys.executable,
                "-m",
                "blueprint_pipeline.measurement_mujoco_flex_cloth_adapter",
            ],
            execute=execute,
        )
        ...
        summaries.append(
            {
                ...
                "maximum_edge_strain": runtime.get("maximum_edge_strain"),
    ...
    if completed:
        sag = [
            _number(
                ...
                "cloth_development_state_trajectory_metric_invalid",
            )
            for row in summaries
        ]
        strain = [
            _number(
                row["maximum_edge_strain"],
                "cloth_development_strain_metric_invalid",
            )
            for row in summaries
        ]
        penetration = [
            _number(
                row["penetration_m"],
                "cloth_development_penetration_metric_invalid",
            )
            for row in summaries
        ]
        aggregate_metrics = {
            "mean_maximum_sag_m": fmean(sag),
            "maximum_sag_m": max(sag),
            "maximum_edge_strain": max(strain),
            "maximum_penetration_m": max(penetration),
            "ground_contact_case_count": sum(
                row["observed_metrics"]["topology_contact"] == "ground_contact" for row in summaries
            ),
        }
    suite = {
        ...
        "lane": "cloth",
        "solver_scope": "mujoco-flex-elastic2d-stretch",
    ...
    return validate_capture_to_deformation_cloth_development_suite(suite)


def validate_capture_to_deformation_cloth_development_suite(
    value: Mapping[str, Any],
) -> dict[str, Any]:
    suite = json.loads(json.dumps(dict(value)))
    errors: list[str] = []
    if suite.get("schema_version") != SUITE_SCHEMA_VERSION:
        errors.append("cloth_development_suite_schema_invalid")
    if suite.get("lane") != "cloth":
        errors.append("cloth_development_suite_lane_invalid")
    if suite.get("solver_scope") != "mujoco-flex-elastic2d-stretch":
        errors.append("cloth_development_suite_solver_scope_invalid")
    if suite.get("status") not in {
        "planned_not_executed",
        "completed_development_only",
        "incomplete",
    }:
        errors.append("cloth_development_suite_status_invalid")
    cases = suite.get("cases")
    if not isinstance(cases, list) or len(cases) < 2:
        errors.append("cloth_development_suite_cases_invalid")
    if suite.get("case_count") != len(cases or []):
        errors.append("cloth_development_suite_case_count_mismatch")
    for key, expected in (
        ("development_only", True),
        ("synthetic_fixture", True),
        ("held_out", False),
    ...
    if suite.get("suite_digest") is not None and suite.get("suite_digest") != expected_digest:
        errors.append("cloth_development_suite_digest_mismatch")
    if errors:
        raise ClothDevelopmentSuiteError(*errors)
...
def main(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Plan or run the Capture-to-Deformation cloth development corpus"
    )
    ...
    suite = run_capture_to_deformation_cloth_development_suite(
        args.corpus,
...
__all__ = [
    ...
    "ClothDevelopmentSuiteError",
    "SUITE_SCHEMA_VERSION",
    "main",
    "run_capture_to_deformation_cloth_development_suite",
    "validate_capture_to_deformation_cloth_development_suite",
]
