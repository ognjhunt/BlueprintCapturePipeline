...
from .mujoco_eval_media import (
    DEFAULT_REVIEW_VIDEO_FPS,
    DEFAULT_VIDEO_FRAME_STRIDE_STEPS,
    counts_by_key as _counts_by_key,
    episode_frame_steps as _episode_frame_steps,
    ffprobe_video as _ffprobe_video,
    review_video_sampling_contract as _review_video_sampling_contract,
    video_output_fps as _video_output_fps,
    video_timing_contract as _video_timing_contract,
)
    ...
    *,
    ...
    if blockers:
        ...
        manifest = {
            ...
            "wam_perception_harness_validation_status": harness_summary.get("validation_status"),
    ...
    for step_index in range(1, max_calls):
        ...
        generated_observation["wam_derived_observation"] = {
            ...
            "bundle_path": harness_summary["artifact_paths"].get("wam_derived_observation_bundle"),
            "checks_path": harness_summary["artifact_paths"].get("wam_perception_harness_checks"),
            ...
            "overall_confidence": _mapping(harness_result["step_record"].get("uncertainty")).get(
                "overall_confidence"
            ),
        ...
        if _mapping(harness_result["step_record"].get("uncertainty")).get(
            ...
            blockers.extend(
                str(item) for item in harness_result["step_record"].get("blockers", []) if str(item)
            )
    ...
    manifest = {
        ...
        "wam_perception_harness_validation_status": harness_summary.get("validation_status"),
        "wam_false_success_reduction_status": harness_summary.get("false_success_reduction_status"),
        "wam_false_success_reduction_rate": harness_summary.get("false_success_reduction_rate"),
    ...
    *,
    ...
    try:
        ...
        if image_module is not None:
            try:
                mask_dir = (
                    job_dir / "policy_segmentation_frames" / str(run.get("episode_id")) / camera_id
                )
                ensure_dir(mask_dir)
        ...
        for episode_index, run in enumerate(matrix["runs"], start=1):
            ...
            for step in range(max(1, int(steps_per_episode))):
                ...
                if step % max(1, int(policy_interval_steps)) == 0:
                    ...
                    visual_observation["segmentation_observation"] = segmentation_observation
