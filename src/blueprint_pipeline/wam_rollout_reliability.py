...
def frame_sequence_motion_series(
    ...
    """Measure the exact ordered RGB evidence without a lossy video round trip."""
...
def assess_rollout_reliability(
    ...
    *,
    ...
    """Screen one encoded generated rollout against its commanded action chunk."""
    ...
    frozen = thresholds or ReliabilityThresholds()
    motion, spatial_std, n_frames = video_motion_series(resolved, frozen.downscale_width)
...
def assess_frame_sequence_reliability(
    ...
    *,
    ...
    """Screen exact generated frames while retaining the frozen thresholds."""

    if timing_flag_scope not in {TIMING_SCOPE_WINDOW, TIMING_SCOPE_SESSION}:
        raise ValueError(f"unsupported_timing_flag_scope:{timing_flag_scope}")
    resolved = [Path(path).expanduser().resolve() for path in frame_paths]
    frozen = thresholds or ReliabilityThresholds()
    motion, spatial_std, n_frames = frame_sequence_motion_series(resolved, frozen.downscale_width)
    return _assess_motion_reliability(
        evidence_path="frame_sequence:" + "|".join(str(path) for path in resolved),
        actions=actions,
        thresholds=frozen,
        timing_flag_scope=timing_flag_scope,
        motion=motion,
