...
#: The flag table *is* the call signature. Both the parser and the keyword
#: arguments are built from this one mapping, and a contract test derives the
#: required left column from `materialize_broad_repair_support` via `inspect`,
#: so a parameter cannot be dropped from the CLI without failing. That matters
#: here because `repair_support_dilation_pixels` selects what actually gets
#: built: a hand-written flag list would fix it at its default invisibly.
CLI_PARAMETERS: dict[str, dict[str, Any]] = {
    "candidate_set_path": {"flag": "--candidate-set", "required": True},
    "renderer_result_path": {"flag": "--renderer-result", "required": True},
    "output_relocation_receipt_path": {"flag": "--relocation-receipt", "required": True},
    "output_root": {"flag": "--output-root", "required": True},
    "repair_support_dilation_pixels": {
        "flag": "--repair-support-dilation-pixels",
        "required": False,
        "type": int,
        "default": 2,
    },
...
def main(argv: Sequence[str] | None = None) -> int:
    """Expose broad repair-support materialization to production CLIs."""

    parser = argparse.ArgumentParser(description=__doc__)
    for name, spec in CLI_PARAMETERS.items():
        parser.add_argument(
            spec["flag"],
            dest=name,
    ...
    result = materialize_broad_repair_support(
        **{name: getattr(args, name) for name in CLI_PARAMETERS}
    )
