...
def _run_teleport_provider(args: argparse.Namespace) -> dict[str, Any]:
    """Canonical Teleport API route; dry-run never loads credentials or mutates."""
    ...
    try:
        request = _load(Path(args.execution_request).expanduser().resolve())
    ...
    try:
        packet = validate_teleport_upload_packet(
            _load(packet_path), packet_root=packet_path.parent
        )
    except Exception as exc:
    ...
    try:
        terms = validate_teleport_terms_review(
            _load(Path(args.terms_review).expanduser().resolve())
        )
    except Exception as exc:
        blockers.append("teleport_terms_review_invalid:" + type(exc).__name__)
        terms = {}
    try:
        observations_value = _load(Path(args.candidate_observations).expanduser().resolve())
        observations = (
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    provider_reconstruction = commands.add_parser("provider-reconstruction")
    provider_reconstruction.add_argument("--provider", choices=("teleport",), default="teleport")
    provider_reconstruction.add_argument("--upload-packet", required=True)
    provider_reconstruction.add_argument("--execution-request", required=True)
    provider_reconstruction.add_argument("--candidate-observations", required=True)
    provider_reconstruction.add_argument("--sealed-evaluation-request")
    provider_reconstruction.add_argument("--output-dir", required=True)
    provider_reconstruction.add_argument(
        "--terms-review",
    ...
    provider_reconstruction.add_argument("--maximum-ply-bytes", type=int, default=4_000_000_000)
    provider_reconstruction.add_argument("--poll-interval-seconds", type=float, default=30.0)
    provider_reconstruction.add_argument("--observed-at", help=argparse.SUPPRESS)
    provider_reconstruction.add_argument("--experimental-branch-diagnostic", action="store_true")
    provider_reconstruction.add_argument("--execute", action="store_true")
    ...
    if args.command == "provider-reconstruction":
        result = _run_teleport_provider(args)
