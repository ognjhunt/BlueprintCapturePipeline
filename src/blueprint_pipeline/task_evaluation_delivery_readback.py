...
def verify_website_delivery(*, run_root, registration, result_delivery, policy_canary_result,
    ...
    expected = {'run_id': registration['run_id'],
    ...
    if (publication.get('status') != 'succeeded'
            or any(publication.get(key) != value for key, value in expected.items()
                   if key != 'operator_registration_digest')):
    ...
    for ids in batches[:max(1, min(int(maximum_batches), 64))]:
        body = {'schema_version': 'task_evaluation_delivery_readback_request.v1',
                'capture_session_id': registration['capture_session_id'], **expected, 'artifact_ids': ids}
        ...
        if (inbox.get('status') != 'verified' or inbox.get('run_id') != expected['run_id']
                ...
                or inbox.get('team_namespace') != registration['team_namespace']
