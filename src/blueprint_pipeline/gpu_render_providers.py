...
class VastRenderProvider(GpuRenderProvider):
    ...
    def worker_log_diagnostic(self, instance_id: str) -> dict:
        """Read a live worker's typed progress without retaining logs or URLs."""
        normalized_id = _normalize_provider_instance_id(instance_id, numeric_only=True)
        key = self._key()
        if normalized_id is None or not key:
            return {"status": "unavailable", "reason": "instance_or_key_missing"}
        from .vast_provider_adapter import _api_json, _fetch_text

        try:
            status, response = _api_json(
                method="PUT", path=f"/instances/request_logs/{normalized_id}/",
        ...
        exception_types = sorted(set(re.findall(
            r"(?m)^\s*(CalledProcessError|FileExistsError|ValueError|RuntimeError|TimeoutError|OSError):",
            log,
        )))
