"""File-bound entrypoint for the hermetic semantic Gaussian lifting baseline."""
...
def run_semantic_gaussian_lifting_stage(
    *,
    request_path: str | Path,
    gaussian_mapping_path: str | Path,
    track_registry_path: str | Path,
    views_path: str | Path,
    output_path: str | Path,
) -> Dict[str, Any]:
    """Verify exact JSON artifacts, run lifting, and atomically write its terminal result."""

    paths = {
        "request": Path(request_path),
        "gaussian_mapping": Path(gaussian_mapping_path),
        "track_registry": Path(track_registry_path),
    ...
    for name in ("gaussian_mapping", "track_registry", "views"):
        payload = _load_json_file(
            paths[name],
            name=name,
            max_bytes=_INPUT_LIMITS[name],
            blockers=blockers,
        )
        if payload is not None and not isinstance(payload, list):
            blockers.append(f"input_json_must_be_array:{name}")
    ...
    if blockers:
        ...
        result = lift_semantic_masks_to_gaussians(
            request,
            gaussian_mapping=payloads["gaussian_mapping"],
            track_registry=payloads["track_registry"],
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Verify and lift tracked masks onto stable Gaussian IDs."
    )
    parser.add_argument("--request", required=True)
    parser.add_argument("--gaussian-mapping", required=True)
    parser.add_argument("--track-registry", required=True)
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    result = run_semantic_gaussian_lifting_stage(
        ...
        track_registry_path=args.track_registry,
