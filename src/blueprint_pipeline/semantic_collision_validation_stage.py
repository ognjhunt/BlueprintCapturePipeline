"""File-bound entrypoint for independent semantic/collision consistency checks."""
...
from .scene_placement.semantic_collision_validation import (
...
_INPUT_LIMITS = {
    "oriented_box_result": 256 * 1024 * 1024,
    "collision_scene": 512 * 1024 * 1024,
}
...
def run_semantic_collision_validation_stage(
    *,
    request_path: str | Path,
    oriented_box_result_path: str | Path,
    collision_scene_path: str | Path,
    output_path: str | Path,
) -> Dict[str, Any]:
    """Verify exact files, cross-check OBBs, and write a terminal result."""

    paths = {
        "request": Path(request_path),
        "oriented_box_result": Path(oriented_box_result_path),
        "collision_scene": Path(collision_scene_path),
    }
    ...
    for name in ("oriented_box_result", "collision_scene"):
        payload = _load_json(paths[name], name=name, limit=_INPUT_LIMITS[name], blockers=blockers)
        if payload is not None and not isinstance(payload, Mapping):
            blockers.append(f"input_json_must_be_object:{name}")
    ...
    if blockers:
        ...
        result = validate_semantic_boxes_against_collision(
            request,
            oriented_box_result=payloads["oriented_box_result"],
            collision_scene=payloads["collision_scene"],
        )
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Cross-check semantic OBB candidates against independent collision evidence."
    )
    parser.add_argument("--request", required=True)
    parser.add_argument("--oriented-box-result", required=True)
    parser.add_argument("--collision-scene", required=True)
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    result = run_semantic_collision_validation_stage(
        ...
        oriented_box_result_path=args.oriented_box_result,
        collision_scene_path=args.collision_scene,
