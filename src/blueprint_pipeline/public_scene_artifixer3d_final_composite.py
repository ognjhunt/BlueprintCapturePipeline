...
def materialize_artifixer3d_final_composite(
    *,
    ...
    if not dual_input_receipt_paths or len(dual_input_receipt_paths) != len(
        raw_result_paths
    ):
    ...
    input_tasks: dict[
        str, tuple[dict[str, Any], dict[str, Any], Path, Path, int]
    ] = {}
    input_records: list[dict[str, Any]] = []
    publisher_scene_id: str | None = None
    for dual_value, raw_value in zip(
        dual_input_receipt_paths, raw_result_paths, strict=True
    ):
        ...
        if (
            ...
            raise ArtiFixer3DFinalCompositeError(
                "artifixer3d_final_input_receipt_invalid"
            )
        ...
        if not scene_id or publisher_scene_id != scene_id:
            raise ArtiFixer3DFinalCompositeError(
                "artifixer3d_final_scene_binding_invalid"
            )
        ...
        if not isinstance(dual_tasks, list) or not isinstance(raw_tasks, list):
            raise ArtiFixer3DFinalCompositeError(
                "artifixer3d_final_task_inventory_invalid"
            )
        if (
            ...
            raise ArtiFixer3DFinalCompositeError(
                "artifixer3d_final_transition_contract_invalid"
            )
        raw_by_id = {
    ...
    for task_id, (task, raw_task, _dual_path, _raw_path, radius) in sorted(
        input_tasks.items()
    ):
        ...
        if not isinstance(frame_rows, list) or not isinstance(raw_frames, list):
            raise ArtiFixer3DFinalCompositeError(
                "artifixer3d_final_frame_inventory_invalid"
            )
        raw_by_index = {
            row.get("frame_index"): row
        ...
        for frame in frame_rows:
            ...
            if (
                ...
                raise ArtiFixer3DFinalCompositeError(
                    "artifixer3d_final_camera_binding_invalid"
                )
            ...
            exact = _image(
                exact_path, mode="L", code="artifixer3d_final_exact_mask_invalid"
            )
            if original.shape != generated.shape or original.shape[:2] != exact.shape:
                raise ArtiFixer3DFinalCompositeError(
                    "artifixer3d_final_frame_shape_invalid"
                )
            ...
            if outside_changes != 0:
                raise ArtiFixer3DFinalCompositeError(
                    "artifixer3d_final_outside_support_changed"
                )
            ...
            Image.fromarray(np.where(support, 255, 0).astype(np.uint8), mode="L").save(
                support_path
            )
