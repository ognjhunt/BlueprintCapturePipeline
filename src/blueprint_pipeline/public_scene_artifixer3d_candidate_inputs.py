...
from .public_scene_repair_preflight_contract import (
    CALIBRATED_RESIDUAL_PREFLIGHT_SCHEMA_VERSION as CALIBRATED_RESIDUAL_PREFLIGHT_SCHEMA,
)
...
def _validated_preflight(path: Path) -> dict[str, Any]:
    ...
    if (
        ...
        or value.get("required_result_checks", {}).get("outside_mask_pixel_delta_required") != 0
        or value.get("required_result_checks", {}).get("locality_mask_dilation_pixels") != 0
    ):
        raise ArtiFixer3DCandidateInputError(["artifixer3d_calibrated_preflight_invalid"])
    return value
...
def _validated_object_absent_reference_receipts(
    ...
    for unresolved in paths:
        path = _file(unresolved, code="artifixer3d_object_absent_reference_receipt_missing")
        value = _read(path, code="artifixer3d_object_absent_reference_receipt_unreadable")
        task_id = str(value.get("task_id") or "")
        frames = value.get("frames")
        if (
            value.get("schema_version") != OBJECT_ABSENT_REFERENCE_SCHEMA
            or value.get("status") != "candidate_frames_exact_support_composited"
        ...
        for frame in frames:
            ...
            if not camera_id or frame.get("outside_support_changed_pixels") != 0:
                raise ArtiFixer3DCandidateInputError(
                    ["artifixer3d_object_absent_reference_receipt_invalid"]
                )
...
def _scene_identity(preflight: Mapping[str, Any]) -> tuple[str, dict[str, Any]]:
    ...
    authority_record = backend.get("execution_authority") if isinstance(backend, Mapping) else None
    ...
    if (
        ...
        raise ArtiFixer3DCandidateInputError(["artifixer3d_execution_authority_invalid"])
    return scene_id, {
...
def _camera_row(row: Any) -> dict[str, Any]:
    ...
    if (
        ...
        raise ArtiFixer3DCandidateInputError(["artifixer3d_frame_shape_or_mask_invalid"])
    ...
    while remaining:
        ...
        next_id = min(
            ...
            key=lambda camera_id: (
                float(
                    np.linalg.norm(remaining[camera_id]["T_world_camera_opencv"][:3, 3] - current)
                ),
                camera_id,
            ),
        )
...
def _write_colmap_points3d(path: Path, splat: Any) -> None:
    ...
    rgb = np.rint(np.clip(0.5 + SH_C0 * np.asarray(splat.f_dc), 0.0, 1.0) * 255.0).astype(np.uint8)
...
def _write_colmap_cameras_and_images(sparse: Path, rows: Sequence[Mapping[str, Any]]) -> None:
...
def _link_or_copy(source: Path, destination: Path) -> None:
    ...
    if _sha256(source) != _sha256(destination):
        raise ArtiFixer3DCandidateInputError(["artifixer3d_colmap_seed_copy_invalid"])
...
def _split_template(
    *,
    ...
    value["split_template_digest"] = canonical_digest(value, digest_field="split_template_digest")
    return value
...
def materialize_object_absent_reference_candidate_receipt(
    *,
    ...
    if (
        ...
        raise ArtiFixer3DCandidateInputError(["artifixer3d_source_candidate_receipt_invalid"])
    ...
    if (
        ...
        raise ArtiFixer3DCandidateInputError(["artifixer3d_object_absent_reference_output_invalid"])
    ...
    for frame in task.get("frames") or []:
        ...
        outside_changes = int(
            np.count_nonzero(np.any(repair[~support] != before[~support], axis=1))
        )
...
def materialize_artifixer3d_candidate_inputs(
    *,
    ...
    for task_id in task_ids:
        ...
        task_rows = _ordered_cameras([row for row in normalized if row["task_id"] == task_id])
        bound_reference = object_absent_receipts.get(task_id)
        bound_frames_by_camera = {
            str(frame["camera_id"]): frame for frame in (bound_reference or {}).get("frames", [])
        }
            ...
            frame_rows.append(
                {
                    ...
                    "binary_opacity_surrogate": _record(opacity_path, root=task_root),
            ...
            split_rows.append(
                {
                    ...
                    "split_template": {
                        **_record(split_path),
                        "split_template_digest": split_template["split_template_digest"],
                    },
                }
            )
        task_receipts.append(
            {
                ...
                "repair_support_coverage": {
                    ...
                    "mean_fraction": sum(repair_support_fractions) / len(repair_support_fractions),
                ...
                "target_indices": _record(target_path) if target_indices else None,
    ...
    receipt: dict[str, Any] = {
        ...
        "source_preflight_replacement_object_count": preflight["replacement_object_count"],
        ...
        "adapter": {
            ...
            "bound_object_absent_reference_task_count": len(object_absent_receipts),
