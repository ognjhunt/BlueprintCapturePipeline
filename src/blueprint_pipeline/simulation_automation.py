...
def _conversion_status(*, framework: str, worldlabs: Mapping[str, Any]) -> Dict[str, Any]:
    collider = _string(worldlabs.get("collider_mesh_glb_url"))
    has_visual = bool(
        worldlabs.get("ply_available") or worldlabs.get("usd_available") or worldlabs.get("spz_available")
    )
    if framework == "isaac_sim":
        if not has_visual:
            status = "blocked"
            blockers = ["missing_visual_asset"]
        elif worldlabs.get("ply_available") or worldlabs.get("usd_available"):
        ...
        blockers = [] if collider else ["missing_collider_mesh_glb"]
...
def _build_asset_conversion_plan(*, plan: Mapping[str, Any], generated_at: str) -> Dict[str, Any]:
    ...
    frameworks = {
        framework: _conversion_status(framework=framework, worldlabs=worldlabs)
