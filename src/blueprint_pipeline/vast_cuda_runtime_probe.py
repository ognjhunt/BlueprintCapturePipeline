...
def cuda_runtime_probe_shell_fragment(*, required: bool, prefer_isaac_python: bool = False) -> str:
    ...
    return (
        ...
        "names.extend(glob.glob('/usr/local/cuda*/lib64/libcudart.so*'))\n"
        "names.extend(glob.glob('/usr/local/cuda*/lib/libcudart.so*'))\n"
        "names.extend(glob.glob('/usr/lib/*-linux-gnu/libcudart.so*'))\n"
        "names.extend(glob.glob('/opt/conda/lib/libcudart.so*'))\n"
        "names.extend(glob.glob('/opt/conda/lib/python*/site-packages/nvidia/cuda_runtime/lib/libcudart.so*'))\n"
        "names.extend(glob.glob('/usr/local/lib/python*/site-packages/nvidia/cuda_runtime/lib/libcudart.so*'))\n"
        ...
        "    if torch_available and torch_count > 0:\n"
        "        print(f'BLUEPRINT_VAST_CUDA_RUNTIME_API_OK:devices={torch_count}:backend=pytorch', flush=True)\n"
        "        raise SystemExit(0)\n"
