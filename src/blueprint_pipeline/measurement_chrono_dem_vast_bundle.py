"""Compile a clean-commit Chrono::DEM CUDA development input bundle."""
...
BUNDLE_SCHEMA_VERSION = "measurement_chrono_dem_cuda_input_bundle.v1"
RECEIPT_SCHEMA_VERSION = "measurement_chrono_dem_cuda_input_bundle_receipt.v1"
RUNNER_RELATIVE_PATH = Path("scripts/run_measurement_chrono_dem_bundle.py")
PROBE_RELATIVE_PATH = Path("scripts/measurement_chrono_dem_cuda_probe.cpp")
PROBE_CMAKE_RELATIVE_PATH = Path("scripts/measurement_chrono_dem_cuda_probe.CMakeLists.txt")
WORKER_RELATIVE_PATH = Path("scripts/measurement_chrono_dem_cuda_worker.py")


class MeasurementChronoDemVastBundleError(ValueError):
    pass
...
def _git_identity(root: Path) -> str:
    ...
    if head.returncode != 0 or status.returncode != 0 or status.stdout.strip():
        raise MeasurementChronoDemVastBundleError("measurement_chrono_dem_source_not_clean_commit")
    commit = head.stdout.strip().lower()
    if len(commit) != 40 or any(character not in "0123456789abcdef" for character in commit):
        raise MeasurementChronoDemVastBundleError("measurement_chrono_dem_source_commit_invalid")
    return commit
...
def _load_corpus(path: Path) -> dict[str, Any]:
    try:
        ...
        raise MeasurementChronoDemVastBundleError(
            "measurement_chrono_dem_corpus_unreadable"
        ) from exc
    if not isinstance(value, Mapping) or value.get("schema_version") != (
        "measurement_chrono_dem_cuda_development_corpus.v1"
    ):
        raise MeasurementChronoDemVastBundleError("measurement_chrono_dem_corpus_invalid")
    if (
        ...
        or value.get("physical_material_characterization_included") is not False
        ...
        raise MeasurementChronoDemVastBundleError("measurement_chrono_dem_corpus_boundary_invalid")
    return dict(value)
    ...
    for index, raw_case in enumerate(corpus["cases"]):
        ...
        requests.append(
            build_measurement_adapter_execution_request(
                ...
                spec,
                case,
                execution_id=f"chrono-dem-cuda-{index + 1:03d}-{case_id}",
...
def compile_measurement_chrono_dem_input_bundle(
    *,
    repo_root: str | Path,
    ...
    if root != corpus_file and root not in corpus_file.parents:
        raise MeasurementChronoDemVastBundleError("measurement_chrono_dem_corpus_outside_source")
    corpus, requests = _requests(
        corpus_file,
        qualification_split_digest=qualification_split_digest,
        controller_scope_digest=controller_scope_digest,
    )
    runtime_release = build_measurement_chrono_dem_runtime_release()
    ...
    source_files.extend(
        ...
        for relative in (
            ...
            PROBE_RELATIVE_PATH,
            PROBE_CMAKE_RELATIVE_PATH,
            WORKER_RELATIVE_PATH,
        )
    )
    if not source_files or any(not path.is_file() or path.is_symlink() for path in source_files):
        raise MeasurementChronoDemVastBundleError("measurement_chrono_dem_source_files_invalid")
    source_records = [
    ...
    manifest = {
        ...
        "chrono_source_repository": runtime_release["chrono_source_repository"],
        ...
        "probe_source_path": PROBE_RELATIVE_PATH.as_posix(),
        "probe_cmake_path": PROBE_CMAKE_RELATIVE_PATH.as_posix(),
    ...
    return validate_measurement_chrono_dem_input_bundle_receipt(receipt)


def validate_measurement_chrono_dem_input_bundle_receipt(
    value: Mapping[str, Any],
) -> dict[str, Any]:
    receipt = json.loads(json.dumps(dict(value)))
    errors: list[str] = []
    if receipt.get("schema_version") != RECEIPT_SCHEMA_VERSION:
        errors.append("measurement_chrono_dem_bundle_receipt_schema_invalid")
    if receipt.get("runtime_image_digest") != RUNTIME_IMAGE:
        errors.append("measurement_chrono_dem_bundle_receipt_image_invalid")
    if receipt.get("chrono_source_commit") != EXPECTED_SOURCE_COMMIT:
        errors.append("measurement_chrono_dem_bundle_receipt_source_invalid")
    if (
        receipt.get("request_count") != 2
        or len(receipt.get("execution_request_digests") or []) != 2
    ):
        errors.append("measurement_chrono_dem_bundle_receipt_requests_invalid")
    if receipt.get("required_backend") != "cuda" or receipt.get("replay_count") != 2:
        errors.append("measurement_chrono_dem_bundle_receipt_runtime_contract_invalid")
    for key, expected in (
        ("raw_secret_values_recorded", False),
        ("provider_allocation_performed", False),
        ("paid_execution_authorized_by_bundle", False),
        ...
        if receipt.get(key) != expected:
            errors.append(f"measurement_chrono_dem_bundle_receipt_{key}_invalid")
    if receipt.get("bundle_receipt_digest") != canonical_digest(
        receipt, digest_field="bundle_receipt_digest"
    ):
        errors.append("measurement_chrono_dem_bundle_receipt_digest_mismatch")
    if errors:
        raise MeasurementChronoDemVastBundleError(";".join(sorted(set(errors))))
    return receipt
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    receipt = compile_measurement_chrono_dem_input_bundle(
